...
/**
 * ONE-TIME: Delete rows with "Error: No slideshow" in column G
 */
function deleteNoSlideshowRows() {
  var ui = SpreadsheetApp.getUi();
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Article Collection');
  ...
  // Find rows to delete (collect from bottom to top)
  var rowsToDelete = [];
  for (var i = intros.length - 1; i >= 0; i--) {
    if (intros[i][0] && intros[i][0].toString().indexOf('Error: No slideshow') !== -1) {
      rowsToDelete.push(i + 2); // Sheet row (1-indexed + header)
    }
  }
  
  if (rowsToDelete.length === 0) {
    ui.alert('None Found', 'No rows with "Error: No slideshow" found.', ui.ButtonSet.OK);
    return;
  }
  
  var response = ui.alert('Delete Rows', 'Found ' + rowsToDelete.length + ' rows with "Error: No slideshow".\n\nDelete them?', ui.ButtonSet.YES_NO);
  if (response !== ui.Button.YES) return;
  ...
  SpreadsheetApp.flush();
  ui.alert('Done', 'Deleted ' + rowsToDelete.length + ' rows.', ui.ButtonSet.OK);
}

/**
 * ONE-TIME: Delete rows without thumbnail URLs in column E
 */
function deleteNoThumbnailRows() {
  var ui = SpreadsheetApp.getUi();
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Article Collection');
  ...
  // Read column E (Thumbnail URL)
  var thumbnails = sheet.getRange(2, 5, lastRow - 1, 1).getValues();
  
  // Find rows to delete (from bottom to top)
  var rowsToDelete = [];
  for (var i = thumbnails.length - 1; i >= 0; i--) {
    if (!thumbnails[i][0] || thumbnails[i][0].toString().trim() === '') {
      rowsToDelete.push(i + 2);
    }
  }
  
  if (rowsToDelete.length === 0) {
    ui.alert('None Found', 'All rows have thumbnail URLs.', ui.ButtonSet.OK);
    return;
  }
  
  var response = ui.alert('Delete Rows', 'Found ' + rowsToDelete.length + ' rows without thumbnail URLs.\n\nDelete them?', ui.ButtonSet.YES_NO);
  if (response !== ui.Button.YES) return;
...
/**
 * Creates the Article Collection sheet with headers and formatting
 */
function createArticleCollectionSheet() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(CONFIG.SHEETS.ARTICLE_COLLECTION);

  // Create sheet if it doesn't exist
  if (!sheet) {
    sheet = ss.insertSheet(CONFIG.SHEETS.ARTICLE_COLLECTION);
  } else {
  ...
  // Set up headers
  var headers = ['State', 'Category', 'Title', 'Article URL', 'Thumbnail URL', 'Used?'];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);

  // Format header row
  sheet.getRange(1, 1, 1, headers.length)
    .setBackground(CONFIG.COLORS.DARK_TEAL)
    .setFontColor(CONFIG.COLORS.WHITE)
    .setFontWeight('bold');

  // Set column widths
  sheet.setColumnWidth(1, 100);  // State
  sheet.setColumnWidth(2, 120);  // Category
  sheet.setColumnWidth(3, 400);  // Title
  sheet.setColumnWidth(4, 300);  // Article URL
  sheet.setColumnWidth(5, 300);  // Thumbnail URL
  ...
  Logger.log('Article Collection sheet created');
  SpreadsheetApp.getUi().alert('Done', 'Article Collection sheet created!', SpreadsheetApp.getUi().ButtonSet.OK);
  
  return sheet;
}

/**
 * Creates the Email Newsletter sheet with headers and formatting
 */
function createEmailNewsletterSheet() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(CONFIG.SHEETS.EMAIL_NEWSLETTER);

  // Create sheet if it doesn't exist
  if (!sheet) {
    sheet = ss.insertSheet(CONFIG.SHEETS.EMAIL_NEWSLETTER);
  } else {
    // Clear existing content
    sheet.clear();
  }

  // Set up headers with divider columns
  var headers = [
    'Send Date',
    'Featured Article', 'URL', 'Image URL', 'Preview Text',
    '', // Divider
    'Article #2', 'URL', 'Image URL',
    '', // Divider
    'Article #3', 'URL', 'Image URL',
    '', // Divider
    'Article #4', 'URL', 'Image URL',
    '', // Divider
    'Status'
  ];
  ...
  // Set column widths
  sheet.setColumnWidth(1, 100);  // Send Date
  sheet.setColumnWidth(2, 300);  // Featured Article
  sheet.setColumnWidth(3, 200);  // URL
  sheet.setColumnWidth(4, 200);  // Image URL
  sheet.setColumnWidth(5, 250);  // Preview Text
  sheet.setColumnWidth(6, 10);   // Divider
  sheet.setColumnWidth(7, 300);  // Article #2
  sheet.setColumnWidth(8, 200);  // URL
  sheet.setColumnWidth(9, 200);  // Image URL
  sheet.setColumnWidth(10, 10);  // Divider
  sheet.setColumnWidth(11, 300); // Article #3
  sheet.setColumnWidth(12, 200); // URL
  sheet.setColumnWidth(13, 200); // Image URL
  sheet.setColumnWidth(14, 10);  // Divider
  sheet.setColumnWidth(15, 300); // Article #4
  sheet.setColumnWidth(16, 200); // URL
  sheet.setColumnWidth(17, 200); // Image URL
  sheet.setColumnWidth(18, 10);  // Divider
  sheet.setColumnWidth(19, 120); // Status

  // Make divider columns black
  var dividerColumns = [6, 10, 14, 18];
  for (var i = 0; i < dividerColumns.length; i++) {
    sheet.getRange(1, dividerColumns[i], 100, 1).setBackground(CONFIG.COLORS.BLACK);
  }

  // Add data validation for Status column
  var statusValidation = SpreadsheetApp.newDataValidation()
    .requireValueInList(['Create Newsletter', 'Pending', 'Sent'], true)
    .build();
  ...
  Logger.log('Email Newsletter sheet created');
  SpreadsheetApp.getUi().alert('Done', 'Email Newsletter sheet created!', SpreadsheetApp.getUi().ButtonSet.OK);
...
function checkWordPressStatus() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var aleksSheet = ss.getSheetByName(CONFIG.SHEETS.WP_EDITING_TRACKER);
  
  var username = CONFIG.WORDPRESS.USERNAME;
  var applicationPassword = CONFIG.WORDPRESS.APP_PASSWORD;
  
  // Get the maximum row that has content
  var lastRow = aleksSheet.getLastRow();
  if (lastRow < 2) {
    Logger.log('No data rows to process');
    return;
  }
  
  // Get all data from row 1 to last row with content
  var dataRange = aleksSheet.getRange(1, 1, lastRow, 15); // Columns A through O
  var values = dataRange.getValues();
  
  // Find the row with "DRAFTS" in Column B
  var maxRowToProcess = lastRow;
  for (var j = 1; j < values.length; j++) {
    if (values[j][1] && String(values[j][1]).includes("DRAFTS")) {
      maxRowToProcess = j;
      Logger.log(`Found DRAFTS marker at row ${j + 1}, will only process rows 2-${j}`);
      break;
    }
  }
  ...
  for (var i = 1; i < maxRowToProcess; i++) {
    ...
    try {
      if (aleksSheet.isRowHiddenByUser(currentRowNumber)) {
        Logger.log(`Skipping row ${currentRowNumber} - Row is hidden`);
        continue;
      }
    } catch (error) {
      Logger.log(`Could not check hidden status for row ${currentRowNumber}, continuing...`);
    }
    
    var wpLink = values[i][2]; // Column C - WP Link
    var status = values[i][6]; // Column G - Status
    
    // ONLY skip if status is EXACTLY "MSN" or "Published - Web"
    if (status) {
      var statusStr = String(status).trim();
      if (statusStr === "MSN" || statusStr === "Published - Web") {
        Logger.log(`Skipping row ${currentRowNumber} - Status is: ${status}`);
        continue;
      }
    }
    
    // Skip if no WordPress link
    if (!wpLink) {
      Logger.log(`Row ${currentRowNumber} - No WordPress link to check`);
      continue;
    }
    
    Logger.log(`Checking row ${currentRowNumber} with status: "${status || 'blank'}"`);
    
    try {
      // Extract post ID
      var postId = extractPostIdFromUrl(wpLink);
      if (!postId) {
        Logger.log(`Could not extract post ID from URL: ${wpLink}`);
        continue;
      }
      ...
      if (wpStatus && wpStatus.status === CONFIG.WP_POST_STATUS.PUBLISH) {
        Logger.log(`Post ${postId} is published! Updating row ${currentRowNumber}...`);
        
        try {
          // Update Final Title (Column E) - Plain text, Arial 12, centered
          var titleCell = aleksSheet.getRange(currentRowNumber, 5);
          
          // Get current background color to preserve it
          var currentTitleBackground = titleCell.getBackground();
          
          // Set the value
          titleCell.setValue(wpStatus.title);
          
          // Apply ONLY text-related formatting
          titleCell.setFontFamily("Arial");
          titleCell.setFontSize(12);
          titleCell.setFontColor(CONFIG.COLORS.BLACK); // Black text
          titleCell.setHorizontalAlignment("center");
          titleCell.setVerticalAlignment("middle");
          titleCell.setFontWeight("normal");
          titleCell.setFontStyle("normal");
          titleCell.setWrap(true);
          
          // Restore the background color
          titleCell.setBackground(currentTitleBackground);
          
          // Remove hyperlink using RichTextValue
          var richValue = SpreadsheetApp.newRichTextValue()
            .setText(wpStatus.title)
            .build();
          titleCell.setRichTextValue(richValue);
          
          Logger.log(`✓ Title updated in Column E`);
          
          // Update Status (Column G) to "Published - Web"
          aleksSheet.getRange(currentRowNumber, 7).setValue("Published - Web");
          Logger.log(`✓ Status updated in Column G`);
          
          // Update Web Article Link (Column H)
          aleksSheet.getRange(currentRowNumber, 8).setValue(wpStatus.link);
          Logger.log(`✓ Web link updated in Column H: ${wpStatus.link}`);
          
          // Update Publish Date & Time (Column I) - Arial 10, LEFT align, CLIP text wrapping
          var publishDate = formatLADateTime(wpStatus.publishDate);
          var dateCell = aleksSheet.getRange(currentRowNumber, 9);
          
          // Get current background color to preserve it
          var currentDateBackground = dateCell.getBackground();
          
          // Set the value
          dateCell.setValue(publishDate);
          
          // Apply ONLY text-related formatting
          dateCell.setFontFamily("Arial");
          dateCell.setFontSize(10); // Size 10 for date
          dateCell.setFontColor(CONFIG.COLORS.BLACK); // Black text
          dateCell.setHorizontalAlignment("left"); // LEFT align instead of center
          dateCell.setVerticalAlignment("middle");
          dateCell.setFontWeight("normal");
          dateCell.setFontStyle("normal");
          dateCell.setWrapStrategy(SpreadsheetApp.WrapStrategy.CLIP); // CLIP instead of wrap
          
          // Restore the background color
          dateCell.setBackground(currentDateBackground);
          
          // Remove any hyperlink
          var dateRichValue = SpreadsheetApp.newRichTextValue()
            .setText(publishDate)
            .build();
          dateCell.setRichTextValue(dateRichValue);
          
          Logger.log(`✓ Publish date updated in Column I: ${publishDate}`);
          
          // Add green check emoji (Column J) - Just the value
          aleksSheet.getRange(currentRowNumber, 10).setValue("✅");
          Logger.log(`✓ Check mark added in Column J`);
          
          // Force save
          SpreadsheetApp.flush();
          
          Logger.log(`=========== Row ${currentRowNumber} UPDATE COMPLETE ===========`);
          
        } catch (updateError) {
          Logger.log(`ERROR updating cells for row ${currentRowNumber}: ${updateError.message}`);
          Logger.log(`Update error stack: ${updateError.stack}`);
        }
        
      } else if (wpStatus) {
        Logger.log(`Row ${currentRowNumber} - WordPress post status is: ${wpStatus.status} (not published yet)`);
      } else {
        Logger.log(`Row ${currentRowNumber} - Could not retrieve post status`);
      }
      
    } catch (error) {
      Logger.log(`Error processing row ${currentRowNumber}: ${error.message}`);
      Logger.log(`Error stack: ${error.stack}`);
    }
  }
  
  Logger.log('WordPress status check completed');
}
...
function diagnoseAdminCapabilities() {
  var username = CONFIG.WORDPRESS.USERNAME;
  var applicationPassword = CONFIG.WORDPRESS.APP_PASSWORD;
  
  Logger.log("Diagnosing Administrator capabilities issue...\n");
  
  // Test 1: Get current user details
  var userUrl = CONFIG.ENDPOINTS.WP_USERS + "/me";
  ...
  try {
    var response = UrlFetchApp.fetch(userUrl, options);
    var userData = JSON.parse(response.getContentText());
    
    Logger.log("=== USER DETAILS ===");
    Logger.log("Username: " + userData.slug);
    Logger.log("Name: " + userData.name);
    Logger.log("ID: " + userData.id);
    Logger.log("Roles: " + JSON.stringify(userData.roles));
    
    // List ALL capabilities
    Logger.log("\n=== ALL CAPABILITIES ===");
    if (userData.capabilities) {
      var caps = Object.keys(userData.capabilities);
      Logger.log("Total capabilities: " + caps.length);
      ...
      importantCaps.forEach(function(cap) {
        var hasCap = userData.capabilities[cap];
        Logger.log("- " + cap + ": " + (hasCap ? "YES ✅" : "NO ❌"));
      });
      
      // Show first 10 capabilities as sample
      Logger.log("\nFirst 10 capabilities:");
      caps.slice(0, 10).forEach(function(cap) {
        Logger.log("- " + cap + ": " + userData.capabilities[cap]);
      });
    } else {
      Logger.log("❌ No capabilities object found!");
    }
    
    // Test 2: Try to access media endpoint
    Logger.log("\n=== TESTING MEDIA ENDPOINT ACCESS ===");
    var mediaUrl = CONFIG.ENDPOINTS.WP_MEDIA;
    var mediaResponse = UrlFetchApp.fetch(mediaUrl, {
      method: "get",
      headers: {
        "Authorization": "Basic " + Utilities.base64Encode(username + ":" + applicationPassword)
      },
    ...
    Logger.log("Media endpoint status: " + mediaResponse.getResponseCode());
    if (mediaResponse.getResponseCode() !== CONFIG.HTTP_STATUS.OK) {
      Logger.log("Media endpoint error: " + mediaResponse.getContentText().substring(0, 200));
    } else {
      Logger.log("✅ Can access media endpoint");
    }
    
    // Test 3: Check context
    Logger.log("\n=== CHECKING CONTEXT ===");
    var contextUrl = CONFIG.ENDPOINTS.WP_USERS + "/me?context=edit";
    var contextResponse = UrlFetchApp.fetch(contextUrl, {
      method: "get",
      headers: {
        "Authorization": "Basic " + Utilities.base64Encode(username + ":" + applicationPassword)
      },
    ...
    if (contextResponse.getResponseCode() === CONFIG.HTTP_STATUS.OK) {
      var contextData = JSON.parse(contextResponse.getContentText());
      Logger.log("Edit context accessible: YES");
      Logger.log("Capabilities in edit context: " + (contextData.capabilities ? "Present" : "Missing"));
    } else {
      Logger.log("Edit context accessible: NO");
    }
...
// Test with a different endpoint that might give more info
function testAlternativeAuth() {
  ...
  // Test creating a simple post (admins should always be able to do this)
  var postData = {
  ...
  try {
    var response = UrlFetchApp.fetch(CONFIG.ENDPOINTS.WP_POSTS, options);
    
    if (response.getResponseCode() === CONFIG.HTTP_STATUS.CREATED) {
      var post = JSON.parse(response.getContentText());
      Logger.log("✅ Can create posts - Post ID: " + post.id);
      ...
      var deleteResponse = UrlFetchApp.fetch(
        CONFIG.ENDPOINTS.WP_POSTS + "/" + post.id + "?force=true",
        deleteOptions
      );
      
      if (deleteResponse.getResponseCode() === CONFIG.HTTP_STATUS.OK) {
        Logger.log("✅ Can delete posts");
      }
    } else {
      Logger.log("❌ Cannot create posts: " + response.getContentText());
    }
  } catch (error) {
    Logger.log("Error testing post creation: " + error.message);
  }
}
...
function setupScheduledPostsMonitor() {
  // Delete existing triggers for this function
  var triggers = ScriptApp.getProjectTriggers();
  triggers.forEach(function(trigger) {
    if (trigger.getHandlerFunction() === 'checkScheduledPostsStatus') {
      ScriptApp.deleteTrigger(trigger);
    }
  });
  
  // Create new trigger to run every hour
  ScriptApp.newTrigger('checkScheduledPostsStatus')
    .timeBased()
    .everyHours(1)
    .create();
    
  Logger.log('Scheduled posts monitor set up - runs every hour');
}
...
function setupProductionTrackerMonitor() {
  // Delete existing triggers for this function
  var triggers = ScriptApp.getProjectTriggers();
  triggers.forEach(function(trigger) {
    if (trigger.getHandlerFunction() === 'monitorProductionTracker') {
      ScriptApp.deleteTrigger(trigger);
    }
  });
  
  // Create new trigger to run every 5 minutes
  ScriptApp.newTrigger('monitorProductionTracker')
    .timeBased()
    .everyMinutes(5)
    .create();
    
  Logger.log('Production Tracker monitor set up - runs every 5 minutes during Phoenix business hours (9 AM - 8 PM)');
}


// Check WP status for all rows in the sheet
function checkAllWPStatuses() {
  ...
  var errorCount = 0;
  var skippedCount = 0; // Added to track skipped rows
  
  // Start from row 2 (assuming row 1 has headers)
  for (var row = 2; row <= lastRow; row++) {
    var wpUrl = sheet.getRange(row, 4).getValue(); // Column D
    var currentStatus = sheet.getRange(row, 8).getValue(); // Column H
    
    // Skip rows where status is "DONE"
    if (currentStatus === CONFIG.STATUS.DONE) {
      skippedCount++;
      continue;
    }
    
    // Only process rows that have a WP URL
    if (wpUrl && wpUrl.toString().trim() !== '') {
      try {
        // Create a mock event object to reuse the existing function
        var mockEvent = {
          range: sheet.getRange(row, 8),
          value: 'Check WP Status'
        };
        
        // Call the existing function
        checkWordPressStatus(mockEvent);
        processedCount++;
  ...
  SpreadsheetApp.getUi().alert(
    'WordPress Status Check Complete\n\n' +
    'Rows processed: ' + processedCount + '\n' +
    'Rows skipped (DONE): ' + skippedCount + '\n' +
    'Errors: ' + errorCount
  );
}
...
// Batch function to set date/time for multiple posts
function batchSetDateTime() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEETS.WP_EDITING_TRACKER);
  if (!sheet) {
    SpreadsheetApp.getUi().alert('WP Editing Tracker sheet not found');
    return;
  ...
  // Find all rows that have date/time but haven't been set yet
  for (var row = 2; row <= lastRow; row++) {
    var wpUrl = sheet.getRange(row, 4).getValue(); // Column D
    var time = sheet.getRange(row, 6).getDisplayValue(); // Column F
    var date = sheet.getRange(row, 7).getDisplayValue(); // Column G
    ...
    // Check if row has URL, date, time, and status indicates it needs date setting
    if (wpUrl && time && date && 
        (status === 'WordPress Draft' || status === 'Ready' || status === 'In Progress')) {
      ...
      postsToProcess.push({
        ...
        time: time,
        date: date,
        title: sheet.getRange(row, 3).getValue() || 'No title' // Column C for reference
      });
    }
  }
  ...
  if (postsToProcess.length === 0) {
    SpreadsheetApp.getUi().alert(
      'No Posts to Process',
      'No posts found that need date/time setting.\n\n' +
      'Posts must have:\n' +
      '• WordPress URL in Column D\n' +
      '• Time in Column F\n' +
      '• Date in Column G\n' +
      '• Status: WordPress Draft, Ready, or In Progress',
      SpreadsheetApp.getUi().ButtonSet.OK
    );
    return;
  }
  ...
  var response = SpreadsheetApp.getUi().alert(
    'Batch Set Date/Time',
    'Found ' + postsToProcess.length + ' posts to set date/time.\n\n' +
    'This will:\n' +
    '• Set the publish date/time in WordPress\n' +
    '• Keep posts as drafts (not scheduled yet)\n' +
    '• Update status to "Date Set"\n\n' +
    'Continue?',
    SpreadsheetApp.getUi().ButtonSet.YES_NO
  );
  ...
  for (var i = 0; i < postsToProcess.length; i++) {
    ...
    try {
      Logger.log('Setting date/time for row ' + post.row + ': ' + post.date + ' ' + post.time);
      ...
      scheduleWordPressPost(mockEvent);
      
      // Check if successful
      var newStatus = sheet.getRange(post.row, 8).getValue();
      if (newStatus === 'Date Set') {
        successCount++;
      } else {
        errorCount++;
      }
      ...
      errorCount++;
      Logger.log('Error processing row ' + post.row + ': ' + error.message);
    }
  }
  
  // Show results
  SpreadsheetApp.getUi().alert(
    'Batch Set Date/Time Complete',
    'Processed ' + postsToProcess.length + ' posts:\n\n' +
...
////BATCH SEND TO EDITORS

// =============================================================================
// BATCH SEND ALL TO EDITORS
// =============================================================================
...
function batchSendAllToEditors() {
  var operationType = 'Batch Send to Editors';
  ...
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var drafterSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
  var statusSheet = ss.getSheetByName(CONFIG.SHEETS.ARTICLE_STATUS_TRACKER);
  
  if (!drafterSheet || !statusSheet) {
  ...
  try {
    // Step 1: Scan Drafter Basic for articles to transfer (BATCH READ)
    var articlesToTransfer = scanDrafterBasicForTransfer(drafterSheet);
    
    if (articlesToTransfer.totalFound === 0) {
      SpreadsheetApp.getUi().alert('No articles found with "For Transfer" status and URL in column P.');
      unlockUploaderSheet(operationType);
      return;
    }
    
    // Step 2: Check for duplicates in Article Status Tracker (BATCH READ)
    var transferPlan = checkForDuplicatesAndPlan(articlesToTransfer.articles, statusSheet);
    
    // Step 3: Show confirmation dialog
    if (!showSendToEditorsConfirmation(transferPlan)) {
      unlockUploaderSheet(operationType);
      return; // User cancelled
    }
    
    // Step 4: Execute the transfer (BATCH WRITE)
    executeSendToEditorsTransfer(transferPlan, drafterSheet, statusSheet, operationType);
    
  } catch (error) {
    Logger.log('Error in batchSendAllToEditors: ' + error.message);
    unlockUploaderSheet(operationType);
    SpreadsheetApp.getUi().alert('Error: ' + error.message);
  }
}

// OPTIMIZED: Batch read all data at once
function scanDrafterBasicForTransfer(drafterSheet) {
  var lastRow = drafterSheet.getLastRow();
  var articles = [];
  var errorCount = 0;
  
  Logger.log('🔍 Scanning Drafter Basic for articles to transfer...');
  
  if (lastRow < 2) {
    return { articles: [], totalFound: 0, errorCount: 0 };
  }
  
  // BATCH READ ALL DATA AT ONCE - much faster!
  var allData = drafterSheet.getRange(2, 1, lastRow-1, 17).getValues(); // Columns A-Q
  var statusUpdates = []; // Collect status updates for batch writing
  
  for (var i = 0; i < allData.length; i++) {
    var row = i + 2; // Actual sheet row number
    var urlColumnP = allData[i][15]; // Column P (index 15)
    var statusColumnQ = allData[i][16]; // Column Q (index 16)
    
    // Check if row has URL and status is "For Transfer"
    if (urlColumnP && statusColumnQ === 'For Transfer') {
      var columnC = allData[i][2]; // Column C (index 2)
      var columnD = allData[i][3]; // Column D (index 3) - filename
      var columnE = allData[i][4]; // Column E (index 4)
      var columnF = allData[i][5]; // Column F (index 5)
      var columnB = allData[i][1]; // Column B (index 1) - place
      var columnM = allData[i][12]; // Column M (index 12) - tags
      
      // Check if required columns have values
      if (!columnD || !columnE || !columnF) {
        errorCount++;
        Logger.log('❌ Row ' + row + ' has URL but missing data in required columns (D, E, or F)');
        // Collect error status update for batch writing
        statusUpdates.push({
          row: row,
          status: 'Error: Missing required data'
        });
      } else {
        // Valid article to transfer  
        articles.push({
          row: row,
          title: columnD.toString().trim(), // Column D (filename) for title
          columnC: columnC || '', // Column C
          columnE: columnE,
          columnF: columnF,
          columnB: columnB || '', // Place (might be empty)
          columnM: columnM || '', // Tags (might be empty)
          url: urlColumnP,
          status: 'ready'
        });
        Logger.log('✅ Found article: ' + articles[articles.length - 1].title);
      }
    }
  }
  
  // BATCH WRITE error status updates if any
  if (statusUpdates.length > 0) {
    batchUpdateDrafterStatus(drafterSheet, statusUpdates);
  }
...
// OPTIMIZED: Batch read existing titles
function checkForDuplicatesAndPlan(articles, statusSheet) {
  var lastRow = statusSheet.getLastRow();
  var existingTitles = new Set();
  var insertStartRow = 2; // Default to row 2 if no data found
  
  if (lastRow >= 2) {
    // BATCH READ ALL EXISTING TITLES AT ONCE
    var allStatusData = statusSheet.getRange(2, 1, lastRow-1, 4).getValues(); // Columns A-D
    
    // Build set of existing titles and find insert position
    for (var i = 0; i < allStatusData.length; i++) {
      var titleInC = allStatusData[i][2]; // Column C (index 2)
      var valueD = allStatusData[i][3]; // Column D (index 3)
      
      if (titleInC) {
        existingTitles.add(titleInC.toString().trim().toLowerCase());
      }
      
      // Find last row with data in both C and D
      if (titleInC && valueD) {
        insertStartRow = i + 3; // +2 for 0-based to 1-based, +1 for next row
      }
    }
  }
  
  Logger.log('📍 Will insert starting at row ' + insertStartRow);
  ...
  for (var i = 0; i < articles.length; i++) {
    ...
    if (existingTitles.has(titleLower)) {
      ...
      Logger.log('🔄 Duplicate found: ' + article.title);
    } else {
      article.status = 'transfer';
      toTransfer.push(article);
    }
  }
  ...
  return {
    ...
    insertStartRow: insertStartRow,
    totalArticles: articles.length
  };
}

function showSendToEditorsConfirmation(transferPlan) {
  var message = '📤 SEND TO EDITORS - BATCH TRANSFER PLAN\n\n';
  
  message += '📊 SUMMARY:\n';
  message += '• Total articles found: ' + transferPlan.totalArticles + '\n';
  message += '• Articles to transfer: ' + transferPlan.toTransfer.length + '\n';
  message += '• Duplicates (will skip): ' + transferPlan.duplicates.length + '\n';
  message += '• Insert starting at row: ' + transferPlan.insertStartRow + '\n\n';
  
  if (transferPlan.duplicates.length > 0) {
    message += '🔄 DUPLICATES FOUND:\n';
    for (var i = 0; i < Math.min(transferPlan.duplicates.length, 5); i++) {
      message += '• ' + transferPlan.duplicates[i].title + '\n';
    }
    if (transferPlan.duplicates.length > 5) {
      message += '... and ' + (transferPlan.duplicates.length - 5) + ' more\n';
    }
    message += '\n';
  }
  
  if (transferPlan.toTransfer.length > 0) {
    message += '✅ ARTICLES TO TRANSFER:\n';
    for (var j = 0; j < Math.min(transferPlan.toTransfer.length, 5); j++) {
      message += '• ' + transferPlan.toTransfer[j].title + '\n';
    }
    if (transferPlan.toTransfer.length > 5) {
      message += '... and ' + (transferPlan.toTransfer.length - 5) + ' more\n';
    }
    message += '\n';
  }
  
  message += '⚠️ The sheet will be LOCKED during processing.\n\n';
  message += 'Continue with the transfer?';
  
  var response = SpreadsheetApp.getUi().alert(
    'Send to Editors Confirmation',
    message,
    SpreadsheetApp.getUi().ButtonSet.YES_NO
  );
...
// OPTIMIZED: Batch write all transfers
function executeSendToEditorsTransfer(transferPlan, drafterSheet, statusSheet, operationType) {
  var successCount = 0;
  var duplicateCount = 0;
  var errorCount = 0;
  
  Logger.log('📤 STARTING BATCH TRANSFER TO EDITORS...');
  
  try {
    // COLLECT ALL UPDATES FOR BATCH WRITING
    var drafterStatusUpdates = [];
    var statusSheetData = [];
    
    // Process duplicates first - collect status updates
    for (var i = 0; i < transferPlan.duplicates.length; i++) {
      var duplicate = transferPlan.duplicates[i];
      drafterStatusUpdates.push({
        row: duplicate.row,
        status: CONFIG.STATUS.DONE
      });
      duplicateCount++;
      Logger.log('🔄 Will mark duplicate as DONE: ' + duplicate.title);
    }
    
    // Process articles to transfer - collect all data for batch insert
    var currentInsertRow = transferPlan.insertStartRow;
    for (var j = 0; j < transferPlan.toTransfer.length; j++) {
      var article = transferPlan.toTransfer[j];
      
      try {
        // Create combined description (E + F + B) → Column K
        var topicPart = article.columnE.toString().trim() + ' - ' + article.columnF.toString().trim();
        var placePart = article.columnB ? article.columnB.toString().trim() : '';
        var combinedDescription = 'TOPIC: ' + topicPart;
        if (placePart) {
          combinedDescription += '\nPLACE: ' + placePart;
        }
        
        // Prepare row data for Status Sheet (columns A, C, D, G, J, K)
        var rowData = new Array(11); // Create array for columns A through K
        rowData[0] = article.columnC || ''; // Column A
        rowData[2] = article.title; // Column C
        rowData[3] = article.url; // Column D
        rowData[6] = 'Not Available Yet'; // Column G
        rowData[9] = article.columnM || ''; // Column J (tags)
        rowData[10] = combinedDescription; // Column K
        
        statusSheetData.push(rowData);
        
        // Mark as DONE in Drafter Basic
        drafterStatusUpdates.push({
          row: article.row,
          status: CONFIG.STATUS.DONE
        });
        ...
        successCount++;
        Logger.log('✅ Prepared for transfer: ' + article.title + ' to row ' + currentInsertRow);
        currentInsertRow++;
        
      } catch (error) {
        errorCount++;
        Logger.log('❌ Error preparing article: ' + article.title + ' - ' + error.message);
        drafterStatusUpdates.push({
          row: article.row,
    ...
    // BATCH WRITE ALL STATUS UPDATES TO DRAFTER SHEET
    if (drafterStatusUpdates.length > 0) {
      batchUpdateDrafterStatus(drafterSheet, drafterStatusUpdates);
    }
    
    // BATCH WRITE ALL NEW ROWS TO STATUS SHEET
    if (statusSheetData.length > 0) {
      var insertRange = statusSheet.getRange(transferPlan.insertStartRow, 1, statusSheetData.length, 11);
      insertRange.setValues(statusSheetData);
      Logger.log('✅ Batch inserted ' + statusSheetData.length + ' rows to Status Sheet');
    }
    ...
    // Show results
    showSendToEditorsResults(successCount, duplicateCount, errorCount, transferPlan.duplicates);
    
  } finally {
    // Always unlock the sheet
    unlockUploaderSheet(operationType);
  }
}

// Helper function for batch updating Drafter status
function batchUpdateDrafterStatus(drafterSheet, statusUpdates) {
  if (statusUpdates.length === 0) return;
  
  // Sort by row number for efficient writing
  statusUpdates.sort(function(a, b) { return a.row - b.row; });
  
  // Check if rows are contiguous for batch writing
  var isContiguous = true;
  for (var i = 1; i < statusUpdates.length; i++) {
    if (statusUpdates[i].row !== statusUpdates[i-1].row + 1) {
      isContiguous = false;
      break;
    }
  }
  
  if (isContiguous && statusUpdates.length > 1) {
    // Write as single batch if rows are contiguous
    var firstRow = statusUpdates[0].row;
    var statusData = statusUpdates.map(function(update) {
      return [update.status];
    });
    drafterSheet.getRange(firstRow, 17, statusUpdates.length, 1).setValues(statusData);
    Logger.log('✅ Batch updated ' + statusUpdates.length + ' status values (contiguous)');
  } else {
    // Write individually if rows are not contiguous (still faster than original)
    for (var i = 0; i < statusUpdates.length; i++) {
      var update = statusUpdates[i];
      drafterSheet.getRange(update.row, 17).setValue(update.status);
    }
    Logger.log('✅ Updated ' + statusUpdates.length + ' status values (individual)');
  }
}

function showSendToEditorsResults(successCount, duplicateCount, errorCount, duplicatesList) {
  var resultMessage = '📤 Send to Editors Complete!\n\n';
  
  resultMessage += '📊 RESULTS:\n';
  resultMessage += '✅ Successfully transferred: ' + successCount + ' articles\n';
  resultMessage += '🔄 Duplicates found (marked as DONE): ' + duplicateCount + ' articles\n';
  
  if (errorCount > 0) {
    resultMessage += '❌ Errors: ' + errorCount + ' articles\n';
  }
  
  resultMessage += '\n📊 Total processed: ' + (successCount + duplicateCount + errorCount) + ' articles\n';
  
  if (duplicatesList.length > 0) {
    resultMessage += '\n🔄 DUPLICATE ARTICLES:\n';
    for (var i = 0; i < Math.min(duplicatesList.length, 10); i++) {
      resultMessage += '• ' + duplicatesList[i].title + '\n';
    }
    if (duplicatesList.length > 10) {
      resultMessage += '... and ' + (duplicatesList.length - 10) + ' more\n';
    }
  }
  
  if (errorCount > 0) {
    resultMessage += '\n⚠️ Check the Apps Script logs for error details.';
  } else if (successCount > 0) {
    resultMessage += '\n🎉 All articles have been sent to editors!';
  }
  
  resultMessage += '\n\n🔓 The sheet is now unlocked.';
  
  SpreadsheetApp.getUi().alert('Send to Editors Results', resultMessage, SpreadsheetApp.getUi().ButtonSet.OK);
  
  Logger.log('Send to Editors complete - Success: ' + successCount + ', Duplicates: ' + duplicateCount + ', Errors: ' + errorCount);
}

////BATCH SPLIT BASE TOPIC


function parseBaseTopics() {
  var operationType = 'Parse Base Topics';
  ...
  // Start the workspace selection loop
  processBaseTopicsWorkspaceLoop(operationType);
}


function parseTopicSummary(text) {
  try {
    // Remove any leading/trailing whitespace
    text = text.trim();
    
    // Replace line breaks with spaces but preserve some structure
    text = text.replace(/\r\n/g, ' ').replace(/\n/g, ' ').replace(/\r/g, ' ');
    
    // Normalize multiple spaces to single spaces
    text = text.replace(/\s+/g, ' ');
    
    Logger.log('🔍 PARSING TEXT: ' + text.substring(0, 200) + '...');
    
    var result = { success: false, topic: '', summary: '', error: '' };
    
    // Method 1: Look for the two keywords and extract content between them
    var topicMatch = text.match(/\*{0,2}\s*topic\s*:?\s*\*{0,2}\s*/i);
    var summaryMatch = text.match(/\*{0,2}\s*summary\s*:?\s*\*{0,2}\s*/i);
    
    if (topicMatch && summaryMatch) {
      var topicStart = topicMatch.index + topicMatch[0].length;
      var summaryStart = summaryMatch.index + summaryMatch[0].length;
      
      // Extract topic (from topic start to summary start)
      if (topicStart < summaryStart) {
        result.topic = text.substring(topicStart, summaryMatch.index).trim();
      }
      
      // Extract summary (from summary start to end)
      result.summary = text.substring(summaryStart).trim();
      
      // Clean up extracted content
      result.topic = result.topic.replace(/\*+/g, '').trim();
      result.summary = result.summary.replace(/\*+/g, '').trim();
      
      // Validation
      if (result.topic.length > 3 && result.summary.length > 10) {
        result.success = true;
        result.patternUsed = 1;
        Logger.log('✅ SUCCESSFULLY PARSED:');
        Logger.log('   Topic: ' + result.topic);
        Logger.log('   Summary: ' + result.summary.substring(0, 100) + '...');
        return result;
      }
    }
    
    // Method 2: Fallback - use simpler regex patterns
    var fallbackPatterns = [
      // Very permissive pattern
      /topic[:\s*]*([^*]+?)\s*\*+\s*summary[:\s*]*(.+?)$/i,
      // Even more permissive
      /topic[:\s*]*(.+?)\s*summary[:\s*]*(.+?)$/i
    ];
    
    for (var j = 0; j < fallbackPatterns.length; j++) {
      var match = text.match(fallbackPatterns[j]);
      if (match && match[1] && match[2]) {
        result.topic = match[1].replace(/\*+/g, '').trim();
        result.summary = match[2].replace(/\*+/g, '').trim();
        
        if (result.topic.length > 3 && result.summary.length > 10) {
          result.success = true;
          result.patternUsed = j + 2;
          Logger.log('✅ FALLBACK PATTERN WORKED:');
          Logger.log('   Topic: ' + result.topic);
          Logger.log('   Summary: ' + result.summary.substring(0, 100) + '...');
          return result;
        }
    ...
    result.error = 'Could not extract topic and summary';
    
    // Provide debugging info
    if (text.toLowerCase().includes('topic') && 
        text.toLowerCase().includes('summary')) {
      result.error += ' (keywords found but extraction failed)';
      
      // Show positions of keywords for debugging
      var topicPos = text.toLowerCase().indexOf('topic');
      var summaryPos = text.toLowerCase().indexOf('summary');
      
      Logger.log('❌ DEBUGGING INFO:');
      Logger.log('   Topic position: ' + topicPos);
      Logger.log('   Summary position: ' + summaryPos);
      Logger.log('   Order check: ' + (topicPos < summaryPos ? 'CORRECT' : 'WRONG'));
    } else {
      result.error += ' (missing required keywords)';
    }
    ...
    return {
      ...
      topic: '',
      summary: '',
      error: 'Parsing exception: ' + e.message
    };
  }
}

// Add a function to manually test a single article
function debugSingleArticle(rowNumber) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var drafterSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
  
  if (!drafterSheet) {
    Logger.log('❌ Sheet "Drafter Basic" not found');
    return;
  }
  
  var cellE = drafterSheet.getRange(rowNumber, 5).getValue();
  if (!cellE) {
    Logger.log('❌ No content in row ' + rowNumber + ', column E');
    return;
  }
  
  Logger.log('🧪 DEBUGGING ROW ' + rowNumber);
  Logger.log('📝 ORIGINAL TEXT: ' + cellE.toString());
  
  var result = parseTopicSummary(cellE.toString());
...
function validateBaseTopicsArticles(drafterSheet, articles) {
  var validArticles = [];
  var invalidArticles = [];
  
  Logger.log('🔍 VALIDATING ' + articles.length + ' ARTICLES FOR PARSING...');
  
  for (var i = 0; i < articles.length; i++) {
    var article = articles[i];
    var cellE = drafterSheet.getRange(article.row, 5).getValue(); // Column E
    
    if (cellE && cellE.toString().trim()) {
      var originalText = cellE.toString();
      var parsed = parseTopicSummary(originalText);
      
      if (parsed.success) {
        article.parsed = parsed;
        validArticles.push(article);
        Logger.log('✅ Row ' + article.row + ': VALID (Pattern ' + parsed.patternUsed + ')');
        Logger.log('   Topic: ' + parsed.topic);
        Logger.log('   Summary: ' + parsed.summary.substring(0, 100) + '...');
      } else {
        article.error = parsed.error || 'Unknown parsing error';
        article.originalText = parsed.originalText;
        invalidArticles.push(article);
        Logger.log('❌ Row ' + article.row + ': INVALID - ' + article.error);
        Logger.log('   Content preview: ' + originalText.substring(0, 150) + '...');
        
        // Try to identify what went wrong
        if (originalText.toLowerCase().includes('topic:') && 
            originalText.toLowerCase().includes('summary:')) {
          Logger.log('   🔍 Contains all keywords but parsing failed - likely format issue');
        } else {
          Logger.log('   🔍 Missing required keywords (topic:, summary:)');
        }
      }
    } else {
      article.error = 'Empty or missing content in column E';
      invalidArticles.push(article);
      Logger.log('❌ Row ' + article.row + ': INVALID - Empty content');
    }
  }
...
// Test function to help debug specific text
function testParseTopicSummary(testText) {
  Logger.log('🧪 TESTING PARSE FUNCTION');
  Logger.log('Input: ' + testText);
  
  var result = parseTopicSummary(testText);
...
function processBaseTopicsWorkspaceLoop(operationType) {
  try {
    // Step 1: Ask user to choose workspace
    var selectedWorkspace = selectSingleWorkspaceForDrafterBasic('Parse Base Topics'); 
    ...
    if (!drafterSheet) {
      SpreadsheetApp.getUi().alert('Sheet "Drafter Basic" not found!');
      unlockUploaderSheet(operationType);
      return;
    }
    
    // Step 2: Find boundaries for selected workspace
    if (selectedWorkspace.isAllWorkspaces) {
      var workspacesWithBoundaries = [];
      for (var i = 0; i < selectedWorkspace.workspaces.length; i++) {
        var workspace = selectedWorkspace.workspaces[i];
        var workspaceWithBoundaries = findDrafterWorkspaceBoundaries(drafterSheet, workspace.personName);
        if (workspaceWithBoundaries) {
          workspacesWithBoundaries.push(workspaceWithBoundaries);
        }
      }
      ...
      selectedWorkspace = findDrafterWorkspaceBoundaries(drafterSheet, selectedWorkspace.personName);
      if (!selectedWorkspace) {
        SpreadsheetApp.getUi().alert('Could not find workspace boundaries for the selected person.');
        askForNextBaseTopicsWorkspace(operationType);
        return;
      }
    }
    
    // Step 3: Scan the selected workspace(s) for articles to process
    var articlesInWorkspace;
    if (selectedWorkspace.isAllWorkspaces) {
      articlesInWorkspace = [];
      for (var i = 0; i < selectedWorkspace.workspaces.length; i++) {
        var workspace = selectedWorkspace.workspaces[i];
        var workspaceArticles = getArticlesForBaseTopics(drafterSheet, workspace);
        articlesInWorkspace = articlesInWorkspace.concat(workspaceArticles);
      }
    } else {
      articlesInWorkspace = getArticlesForBaseTopics(drafterSheet, selectedWorkspace);
    }
    
    if (articlesInWorkspace.length === 0) {
      SpreadsheetApp.getUi().alert('No articles found with "Base Topic Pasted" status in the selected workspace(s).');
      askForNextBaseTopicsWorkspace(operationType);
      return;
    }
    
    // Step 4: Show plan and process
    if (!showBaseTopicsPlan(selectedWorkspace, articlesInWorkspace)) {
      askForNextBaseTopicsWorkspace(operationType);
      return;
    }
    
    var result = processBaseTopicsWorkspace(selectedWorkspace, articlesInWorkspace);
    showBaseTopicsResultsAndAskNext(result, operationType);
    
  } catch (error) {
    Logger.log('Error in processBaseTopicsWorkspaceLoop: ' + error.message);
    unlockUploaderSheet(operationType);
    SpreadsheetApp.getUi().alert('Error: ' + error.message);
  }
}

// Helper functions for Base Topics
function getArticlesForBaseTopics(drafterSheet, workspace) {
  var articles = [];
  var endRow = workspace.endRow || drafterSheet.getLastRow();
  
  Logger.log('🔍 SCANNING WORKSPACE: ' + workspace.personName + ' for Base Topics');
  
  for (var row = workspace.startRow; row <= endRow; row++) {
    var statusQ = drafterSheet.getRange(row, 17).getValue(); // Column Q
    var cellB = drafterSheet.getRange(row, 2).getValue(); // Column B
    var cellE = drafterSheet.getRange(row, 5).getValue(); // Column E
    
    if (statusQ === "Base Topic Pasted" && cellB && cellE) {
      articles.push({
        row: row,
        statusQ: statusQ,
        workspace: workspace.personName
      });
      Logger.log('📝 FOUND BASE TOPIC: Row ' + row + ' (Status: ' + statusQ + ')');
    }
  }
...
function showBaseTopicsPlan(workspace, articles) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var drafterSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
  ...
  if (workspace.isAllWorkspaces) {
    ...
    for (var w = 0; w < workspace.workspaces.length; w++) {
      var singleWorkspace = workspace.workspaces[w];
      var workspaceArticles = getArticlesForBaseTopics(drafterSheet, singleWorkspace);
      var workspaceValidation = validateBaseTopicsArticles(drafterSheet, workspaceArticles);
      allValid = allValid.concat(workspaceValidation.valid);
      allInvalid = allInvalid.concat(workspaceValidation.invalid);
    }
    ...
    validation = validateBaseTopicsArticles(drafterSheet, articles);
  }
  
  var message = '📝 PARSE BASE TOPICS PLAN\n\n';
  ...
  message += '📊 Total articles found: ' + articles.length + '\n';
  message += '✅ Valid articles (can be parsed): ' + validation.valid.length + '\n';
  message += '❌ Invalid articles (cannot be parsed): ' + validation.invalid.length + '\n\n';
  
  if (validation.valid.length > 0) {
    message += '📋 VALID ARTICLES:\n';
    for (var i = 0; i < Math.min(validation.valid.length, 5); i++) {
      var article = validation.valid[i];
      message += '• Row ' + article.row + ': ' + article.parsed.topic.substring(0, 40) + '...\n';
    }
    
    if (validation.valid.length > 5) {
      message += '... and ' + (validation.valid.length - 5) + ' more valid articles\n';
    }
  }
  
  if (validation.invalid.length > 0) {
    message += '\n❌ INVALID ARTICLES:\n';
    for (var j = 0; j < Math.min(validation.invalid.length, 5); j++) {
      var invalidArticle = validation.invalid[j];
      message += '• Row ' + invalidArticle.row + ': ' + invalidArticle.error + '\n';
    }
    
    if (validation.invalid.length > 5) {
      message += '... and ' + (validation.invalid.length - 5) + ' more invalid articles\n';
    }
  }
  
  message += '\n🔄 PROCESSING PLAN:\n';
  message += '• Parse TOPIC and SUMMARY from column E\n';
  message += '• Split data into columns E and F\n';
  message += '• Create formatted content in column G\n';
  message += '• Apply green background color\n';
  message += '• Update status to "Split"\n';
  message += '• Only ' + validation.valid.length + ' articles will be processed\n\n';
  
  if (validation.valid.length === 0) {
    message += '⚠️ WARNING: No valid articles to process!\n\n';
    message += 'Would you like to see the validation details and try another workspace?';
  } else {
    message += 'Continue with processing ' + validation.valid.length + ' valid articles?';
  }
  
  var response = SpreadsheetApp.getUi().alert(
    'Parse Base Topics Plan',
    message,
    SpreadsheetApp.getUi().ButtonSet.YES_NO
  );
  
  // Store validation results for use in processing
  if (workspace.isAllWorkspaces) {
    workspace.validationResults = validation;
  } else {
    workspace.validationResults = validation;
  }
...
function processBaseTopicsWorkspace(workspace, articles) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var drafterSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
  var processedCount = 0;
  var errorCount = 0;
  
  Logger.log('📝 PROCESSING WORKSPACE: ' + workspace.personName);
  ...
  if (workspace.isAllWorkspaces) {
    for (var i = 0; i < workspace.workspaces.length; i++) {
      ...
      if (workspaceArticles.length > 0) {
        // Validate articles for this workspace
        var validation = validateBaseTopicsArticles(drafterSheet, workspaceArticles);
        
        if (validation.valid.length > 0) {
          Logger.log('📝 PROCESSING: ' + singleWorkspace.personName + ' (' + validation.valid.length + ' valid articles)');
          var workspaceResult = processBaseTopicsArticles(drafterSheet, validation.valid);
          processedCount += workspaceResult.processed;
          errorCount += workspaceResult.errors;
        }
      }
    }
  } else {
    // Use validation results if available, otherwise validate now
    var validation = workspace.validationResults;
    if (!validation) {
      Logger.log('📝 No cached validation, validating now...');
      validation = validateBaseTopicsArticles(drafterSheet, articles);
    }
    
    if (validation.valid.length > 0) {
      Logger.log('📝 Processing ' + validation.valid.length + ' valid articles...');
      var result = processBaseTopicsArticles(drafterSheet, validation.valid);
      processedCount = result.processed;
      errorCount = result.errors;
    } else {
      Logger.log('⚠️ No valid articles to process');
    }
  }
  ...
  return {
    ...
    errors: errorCount,
    totalArticles: articles.length,
    validArticles: workspace.validationResults ? workspace.validationResults.valid.length : 0,
    invalidArticles: workspace.validationResults ? workspace.validationResults.invalid.length : 0
  };
}

function processBaseTopicsArticles(drafterSheet, validatedArticles) {
  // Get all data from the sheet
  var lastRow = drafterSheet.getLastRow();
  var range = drafterSheet.getRange(2, 1, lastRow - 1, 17); // Columns A-Q
  ...
  Logger.log('📝 PROCESSING ' + validatedArticles.length + ' VALIDATED ARTICLES...');
  
  // Process each validated article (these already have parsed data)
  for (var i = 0; i < validatedArticles.length; i++) {
    var article = validatedArticles[i];
    var dataRowIndex = article.row - 2; // Convert to 0-based array index
    
    Logger.log('🔄 Processing Row ' + article.row + '...');
    
    if (dataRowIndex >= 0 && dataRowIndex < data.length) {
      var row = data[dataRowIndex];
      var cellB = row[1]; // Column B (Place)
      var cellE = row[4]; // Column E (Original content)
      
      if (cellB && article.parsed && article.parsed.success) {
        try {
          var parsed = article.parsed; // Use the pre-parsed data
          
          // Update the array values
          row[4] = parsed.topic;   // Column E gets topic (overwrites input)
          row[5] = parsed.summary; // Column F gets summary
          
          // Create formatted content for column G
          var formattedContent = 'Place: ' + cellB + '\n' + 
                                'Historical Event: ' + parsed.topic + ' - ' + parsed.summary;
          row[6] = formattedContent; // Column G
          
          // Update status to "Split"
          row[16] = 'Split'; // Column Q
          
          // Mark this row for background color changes
          rowsToColor.push(article.row);
          
          processedCount++;
          Logger.log('✅ Successfully processed Row ' + article.row);
          
        } catch (e) {
          Logger.log('❌ Error processing Row ' + article.row + ': ' + e.message);
          row[5] = 'Processing Error: ' + e.message;
          errorCount++;
        }
      } else {
        Logger.log('❌ Missing data for Row ' + article.row + ' (cellB: ' + !!cellB + ', parsed: ' + !!article.parsed + ')');
        errorCount++;
      }
    } else {
      Logger.log('❌ Invalid row index for Row ' + article.row);
      errorCount++;
    }
  }
  
  Logger.log('📝 Writing ' + data.length + ' rows back to sheet...');
  ...
  try {
    ...
    Logger.log('❌ Error writing data: ' + e.message);
    return {
      processed: 0,
      errors: validatedArticles.length
    };
  }
  
  // Apply background colors to successfully processed rows
  Logger.log('🎨 Applying colors to ' + rowsToColor.length + ' rows...');
  for (var j = 0; j < rowsToColor.length; j++) {
    try {
      var rowNum = rowsToColor[j];
      
      // Set background color for columns B, E, F
      drafterSheet.getRange(rowNum, 2).setBackground(CONFIG.COLORS.SAGE_GREEN); // Column B
      drafterSheet.getRange(rowNum, 5).setBackground(CONFIG.COLORS.SAGE_GREEN); // Column E
      drafterSheet.getRange(rowNum, 6).setBackground(CONFIG.COLORS.SAGE_GREEN); // Column F

      
      Logger.log('✅ Applied colors to Row ' + rowNum);
    } catch (e) {
      Logger.log('❌ Error applying colors to Row ' + rowNum + ': ' + e.message);
    }
  }
  
  Logger.log('📝 PROCESSING COMPLETE: ' + processedCount + ' processed, ' + errorCount + ' errors');
...
function showBaseTopicsResultsAndAskNext(result, operationType) {
  var resultMessage = '📝 PARSE BASE TOPICS COMPLETE!\n\n';
  resultMessage += '👤 Workspace: ' + result.workspaceName + '\n';
  resultMessage += '📊 Total articles found: ' + result.totalArticles + '\n';
  resultMessage += '✅ Valid articles: ' + result.validArticles + '\n';
  resultMessage += '❌ Invalid articles: ' + result.invalidArticles + '\n';
  resultMessage += '🔄 Successfully processed: ' + result.processed + ' articles\n';
  
  if (result.errors > 0) {
    resultMessage += '⚠️ Processing errors: ' + result.errors + ' articles\n';
  }
  
  if (result.processed > 0) {
    resultMessage += '\n🎉 Base topics successfully parsed!\n';
    resultMessage += '📌 Data split into columns E, F, G with green backgrounds.\n';
    resultMessage += '📌 Status updated to "Split".\n';
  }
  
  if (result.invalidArticles > 0) {
    resultMessage += '\n⚠️ Note: ' + result.invalidArticles + ' articles could not be parsed due to format issues.\n';
    resultMessage += 'Check the execution log for details on invalid articles.\n';
  }
...
function askForNextBaseTopicsWorkspace(operationType) {
  var response = SpreadsheetApp.getUi().alert(
    'No Articles Found',
    'Would you like to try another workspace?',
    SpreadsheetApp.getUi().ButtonSet.YES_NO
...
// Clean Up Outline function
// Clean Up Outline function - UPDATED VERSION
// Clean Up Outline function - UPDATED VERSION

function cleanUpOutline() {
  var operationType = 'Clean Up Outline';
  ...
  // Start the workspace selection loop
  processCleanOutlineWorkspaceLoop(operationType);
}

function processCleanOutlineWorkspaceLoop(operationType) {
  try {
    // Step 1: Ask user to choose workspace
    var selectedWorkspace = selectSingleWorkspaceForDrafterBasic('Clean Up Outline'); 
    ...
    if (selectedWorkspace.isAllWorkspaces) {
      ...
      if (!selectedWorkspace) {
        SpreadsheetApp.getUi().alert('Could not find workspace boundaries for the selected person.');
        askForNextCleanOutlineWorkspace(operationType);
        return;
      }
    }
    ...
    if (selectedWorkspace.isAllWorkspaces) {
      ...
      for (var i = 0; i < selectedWorkspace.workspaces.length; i++) {
        var workspace = selectedWorkspace.workspaces[i];
        var workspaceArticles = getArticlesForCleanOutline(drafterSheet, workspace);
        articlesInWorkspace = articlesInWorkspace.concat(workspaceArticles);
      }
    } else {
      articlesInWorkspace = getArticlesForCleanOutline(drafterSheet, selectedWorkspace);
    }
    
    if (articlesInWorkspace.length === 0) {
      SpreadsheetApp.getUi().alert('No articles found with "Messy Outline" status in the selected workspace(s).');
      askForNextCleanOutlineWorkspace(operationType);
      return;
    }
    
    // Step 4: Show plan and process
    if (!showCleanOutlinePlan(selectedWorkspace, articlesInWorkspace)) {
      askForNextCleanOutlineWorkspace(operationType);
      return;
    }
    
    var result = processCleanOutlineWorkspace(selectedWorkspace, articlesInWorkspace);
    showCleanOutlineResultsAndAskNext(result, operationType);
    
  } catch (error) {
    Logger.log('Error in processCleanOutlineWorkspaceLoop: ' + error.message);
    unlockUploaderSheet(operationType);
    SpreadsheetApp.getUi().alert('Error: ' + error.message);
  }
}

// Helper functions for Clean Outline
function getArticlesForCleanOutline(drafterSheet, workspace) {
  var articles = [];
  var endRow = workspace.endRow || drafterSheet.getLastRow();
  
  Logger.log('🔍 SCANNING WORKSPACE: ' + workspace.personName + ' for Clean Outline');
  
  for (var row = workspace.startRow; row <= endRow; row++) {
    var statusQ = drafterSheet.getRange(row, 17).getValue(); // Column Q
    var cellH = drafterSheet.getRange(row, 8).getValue(); // Column H
    
    if (statusQ === "Messy Outline" && cellH) {
      articles.push({
        row: row,
        statusQ: statusQ,
        workspace: workspace.personName
      });
      Logger.log('🧹 FOUND MESSY OUTLINE: Row ' + row + ' (Status: ' + statusQ + ')');
    }
  }
...
function showCleanOutlinePlan(workspace, articles) {
  var message = '🧹 CLEAN UP OUTLINE PLAN\n\n';
  ...
  if (workspace.isAllWorkspaces) {
    ...
    message += '📋 ARTICLES FOUND (Status: "Messy Outline"):\n';
    for (var i = 0; i < Math.min(articles.length, 8); i++) {
      message += '• Row ' + articles[i].row + '\n';
    }
  ...
  message += '\n🔄 PROCESSING:\n';
  message += '• Extract clean outline from column H\n';
  message += '• Remove extraneous text and format properly\n';
  message += '• Keep only slide headers and bullet points\n';
  message += '• Apply green background color\n';
  message += '• Update status to "Outline Cleaned"\n\n';
...
function processCleanOutlineWorkspace(workspace, articles) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var drafterSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
  var processedCount = 0;
  var errorCount = 0;
  ...
  if (workspace.isAllWorkspaces) {
    for (var i = 0; i < workspace.workspaces.length; i++) {
      var singleWorkspace = workspace.workspaces[i];
      var workspaceArticles = getArticlesForCleanOutline(drafterSheet, singleWorkspace);
      
      if (workspaceArticles.length > 0) {
        Logger.log('🧹 PROCESSING: ' + singleWorkspace.personName + ' (' + workspaceArticles.length + ' articles)');
        var workspaceResult = processCleanOutlineArticles(drafterSheet, workspaceArticles);
        processedCount += workspaceResult.processed;
        errorCount += workspaceResult.errors;
      }
    }
  } else {
    var result = processCleanOutlineArticles(drafterSheet, articles);
    processedCount = result.processed;
    errorCount = result.errors;
  }
...
function processCleanOutlineArticles(drafterSheet, articles) {
  var processedCount = 0;
  var errorCount = 0;
  ...
  for (var i = 0; i < articles.length; i++) {
    ...
    try {
      var cellH = drafterSheet.getRange(row, 8).getValue();
      
      if (cellH) {
        var content = cellH.toString();
        var cleanedOutline = extractOutline(content);
        
        if (cleanedOutline) {
          // Replace content in column H with cleaned outline
          var cellRange = drafterSheet.getRange(row, 8);
          cellRange.setValue(cleanedOutline);
          
          // Change background color to light green
          cellRange.setBackground(CONFIG.COLORS.LIGHT_GREEN);
          
          // Update status in column Q to "Outline Cleaned"
          drafterSheet.getRange(row, 17).setValue('Outline Cleaned');
          
          processedCount++;
          Logger.log('✅ CLEANED OUTLINE: Row ' + row);
        } else {
          errorCount++;
          Logger.log('❌ FAILED TO CLEAN: Row ' + row + ' - Could not extract valid outline');
        }
      }
    } catch (error) {
      errorCount++;
...
function showCleanOutlineResultsAndAskNext(result, operationType) {
  var resultMessage = '🧹 CLEAN UP OUTLINE COMPLETE!\n\n';
  resultMessage += '👤 Workspace: ' + result.workspaceName + '\n';
  resultMessage += '✅ Successfully processed: ' + result.processed + ' articles\n';
  ...
  if (result.processed > 0) {
    resultMessage += '\n🎉 Outlines successfully cleaned!\n';
    resultMessage += '📌 Column H now contains clean outlines with green background.\n';
    resultMessage += '📌 Status updated to "Outline Cleaned".\n';
  }
...
// Helper function to extract and clean the outline
function extractOutline(content) {
  var slides = [];
  var lines = content.split('\n');
  var currentSlide = null;
  var foundOutlineStart = false;
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('**Slide ') && line.includes(':')) {
      foundOutlineStart = true;
      
      // If we have a previous slide, add it to the array
      if (currentSlide) {
    ...
    // Check if this is a bullet point (starts with em dash)
    else if (foundOutlineStart && line.startsWith('—')) {
      if (currentSlide) {
        currentSlide.items.push(line);
      }
    }
    // If we've found the outline and hit something that's not a slide or bullet, we might be done
    else if (foundOutlineStart && !line.startsWith('**') && !line.startsWith('—')) {
      // Check if this might be the end of the outline
      if (line.toLowerCase().includes('would you like') || 
          line.toLowerCase().includes('proceed with') ||
          line.toLowerCase().includes('approved outline')) {
        break; // Stop processing, we've hit the end
      }
    }
  }
  ...
  // If no slides found, return null
  if (slides.length === 0) {
    return null;
  }
  
  // Build the cleaned outline
  var cleanedLines = [];
  for (var j = 0; j < slides.length; j++) {
    var slide = slides[j];
    
    // Add the slide header
    cleanedLines.push(slide.header);
    
    // Add each bullet point
    for (var k = 0; k < slide.items.length; k++) {
      cleanedLines.push(slide.items[k]);
    }
  }
...
function askForNextCleanOutlineWorkspace(operationType) {
  var response = SpreadsheetApp.getUi().alert(
    'No Articles Found',
    'Would you like to try another workspace?',
    SpreadsheetApp.getUi().ButtonSet.YES_NO
...
function parseVisitDetails() {
  var operationType = 'Parse Visit Details';
  ...
  // Start the workspace selection loop
  processVisitDetailsWorkspaceLoop(operationType);
}

function extractSectionItems(content, startMarker, endMarker) {
  var items = [];
  
  try {
    // Create case-insensitive regex pattern for finding the section
    var startPattern = new RegExp(startMarker, 'i');
    var startMatch = content.search(startPattern);
    
    if (startMatch === -1) {
      Logger.log('Start marker not found: ' + startMarker);
      return items;
    }
    
    // Find the actual start of content (after the header line)
    var headerEndIndex = content.indexOf('\n', startMatch);
    if (headerEndIndex === -1) {
      // No newline found, maybe the header is at the end
      return items;
    }
    ...
    var sectionContent;
    if (endMarker) {
      var endPattern = new RegExp(endMarker, 'i');
      var endMatch = content.search(endPattern);
      
      if (endMatch > headerEndIndex) {
        // Extract content between header end and next section start
        sectionContent = content.substring(headerEndIndex + 1, endMatch);
      } else {
        // If end marker not found or comes before header end, take from header end to content end
        sectionContent = content.substring(headerEndIndex + 1);
      }
    } else {
      // No end marker specified, take from header end to end of content
      sectionContent = content.substring(headerEndIndex + 1);
    }
    
    // Clean up the section content first
    sectionContent = sectionContent.trim();
    
    // Split into lines and process each line
    var lines = sectionContent.split(/\r?\n/);
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      // Skip lines that look like section headers (in case there are nested headers)
      if (/\*{0,2}\s*(intro\s+visit\s+details|visiting\s+details)\s*\*{0,2}/i.test(line)) {
        continue;
      }
      
      // Look for lines that start with em dash or other bullet indicators
      var bulletPatterns = [
        /^—\s*(.+)$/,           // Em dash (preferred format)
        /^-\s*(.+)$/,           // Hyphen
        /^\*\s*(.+)$/,          // Asterisk
        /^•\s*(.+)$/,           // Bullet point
      ];
      ...
      for (var j = 0; j < bulletPatterns.length; j++) {
        var match = line.match(bulletPatterns[j]);
        if (match && match[1]) {
          cleanedItem = '— ' + match[1].trim(); // Standardize to em dash
          foundMatch = true;
          break;
        }
      }
      
      // If no bullet pattern found, check if it's substantial content that should be included
      if (!foundMatch && line.length > 10) {
        // Avoid lines that look like random text, headers, or formatting
        var isValidContent = !/^[*#~]{2,}/.test(line) && // Not formatting symbols
                           !/^[A-Z\s]{10,}:?\s*$/.test(line) && // Not all caps headers  
                           !/^(based on|I'll|according to|here)/i.test(line) && // Not intro text
                           /[a-z]/.test(line); // Contains lowercase letters (likely content)
        
        if (isValidContent) {
          cleanedItem = '— ' + line.trim(); // Add em dash for consistency
          foundMatch = true;
        }
      }
      
      if (foundMatch && cleanedItem) {
        items.push(cleanedItem);
      }
    }
    
    Logger.log('Extracted ' + items.length + ' items from section: ' + startMarker);
    
  } catch (error) {
    Logger.log('Error extracting section items: ' + error.message);
  }
  
  return items;
}

function processVisitDetailsWorkspaceLoop(operationType) {
  try {
    // Step 1: Ask user to choose workspace
    var selectedWorkspace = selectSingleWorkspaceForDrafterBasic('Parse Visit Details'); 
    ...
    if (selectedWorkspace.isAllWorkspaces) {
      ...
      if (!selectedWorkspace) {
        SpreadsheetApp.getUi().alert('Could not find workspace boundaries for the selected person.');
        askForNextVisitDetailsWorkspace(operationType);
        return;
      }
    }
    ...
    if (selectedWorkspace.isAllWorkspaces) {
      ...
      for (var i = 0; i < selectedWorkspace.workspaces.length; i++) {
        var workspace = selectedWorkspace.workspaces[i];
        var workspaceArticles = getArticlesForVisitDetails(drafterSheet, workspace);
        articlesInWorkspace = articlesInWorkspace.concat(workspaceArticles);
      }
    } else {
      articlesInWorkspace = getArticlesForVisitDetails(drafterSheet, selectedWorkspace);
    }
    
    if (articlesInWorkspace.length === 0) {
      SpreadsheetApp.getUi().alert('No articles found with "Visit Details Pasted" status in the selected workspace(s).');
      askForNextVisitDetailsWorkspace(operationType);
      return;
    }
    
    // Step 4: Show plan and process
    if (!showVisitDetailsPlan(selectedWorkspace, articlesInWorkspace)) {
      askForNextVisitDetailsWorkspace(operationType);
      return;
    }
    
    var result = processVisitDetailsWorkspace(selectedWorkspace, articlesInWorkspace);
    showVisitDetailsResultsAndAskNext(result, operationType);
    
  } catch (error) {
    Logger.log('Error in processVisitDetailsWorkspaceLoop: ' + error.message);
    unlockUploaderSheet(operationType);
    SpreadsheetApp.getUi().alert('Error: ' + error.message);
  }
}

// Helper functions for Visit Details
function getArticlesForVisitDetails(drafterSheet, workspace) {
  var articles = [];
  var endRow = workspace.endRow || drafterSheet.getLastRow();
  
  Logger.log('🔍 SCANNING WORKSPACE: ' + workspace.personName + ' for Visit Details');
  
  for (var row = workspace.startRow; row <= endRow; row++) {
    var statusQ = drafterSheet.getRange(row, 17).getValue(); // Column Q
    var cellI = drafterSheet.getRange(row, 9).getValue(); // Column I
    
    if (statusQ === "Visit Details Pasted" && cellI) {
      // Check if content has the expected markers
      var content = cellI.toString();
      var hasIntroMarker = /\*{0,2}\s*intro\s+visit\s+details\s*\*{0,2}/i.test(content);
      var hasVisitingMarker = /\*{0,2}\s*visiting\s+details\s*\*{0,2}/i.test(content);
      
      if (hasIntroMarker && hasVisitingMarker) {
        articles.push({
          row: row,
          statusQ: statusQ,
          workspace: workspace.personName
        });
        Logger.log('🚙 FOUND VISIT DETAILS: Row ' + row + ' (Status: ' + statusQ + ')');
      }
    }
  }
...
function showVisitDetailsPlan(workspace, articles) {
  var message = '🚙 PARSE VISIT DETAILS PLAN\n\n';
  ...
  if (workspace.isAllWorkspaces) {
    ...
    message += '📋 ARTICLES FOUND (Status: "Visit Details Pasted"):\n';
    for (var i = 0; i < Math.min(articles.length, 8); i++) {
      message += '• Row ' + articles[i].row + '\n';
    }
  ...
  message += '\n🔄 PROCESSING:\n';
  message += '• Extract INTRO VISIT DETAILS to column J\n';
  message += '• Extract VISITING DETAILS to column I\n';
  message += '• Apply green background colors\n';
  message += '• Update status to "Visit Formatted"\n\n';
  
  message += 'Continue with this workspace?';
  
  var response = SpreadsheetApp.getUi().alert(
    'Parse Visit Details Plan',
    message,
    SpreadsheetApp.getUi().ButtonSet.YES_NO
  );
...
function processVisitDetailsWorkspace(workspace, articles) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var drafterSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
  var processedCount = 0;
  var errorCount = 0;
  ...
  if (workspace.isAllWorkspaces) {
    for (var i = 0; i < workspace.workspaces.length; i++) {
      var singleWorkspace = workspace.workspaces[i];
      var workspaceArticles = getArticlesForVisitDetails(drafterSheet, singleWorkspace);
      
      if (workspaceArticles.length > 0) {
        Logger.log('🚙 PROCESSING: ' + singleWorkspace.personName + ' (' + workspaceArticles.length + ' articles)');
        var workspaceResult = processVisitDetailsArticles(drafterSheet, workspaceArticles);
        processedCount += workspaceResult.processed;
        errorCount += workspaceResult.errors;
      }
    }
  } else {
    var result = processVisitDetailsArticles(drafterSheet, articles);
    processedCount = result.processed;
    errorCount = result.errors;
  }
...
function processVisitDetailsArticles(drafterSheet, articles) {
  var processedCount = 0;
  var errorCount = 0;
  ...
  for (var i = 0; i < articles.length; i++) {
    ...
    try {
      var cellI = drafterSheet.getRange(row, 9).getValue();
      
      if (cellI) {
        var content = cellI.toString();
        
        // Extract INTRO VISIT DETAILS section
        var introItems = extractSectionItems(content, '\\*{0,2}\\s*intro\\s+visit\\s+details\\s*\\*{0,2}', '\\*{0,2}\\s*visiting\\s+details\\s*\\*{0,2}');
        
        // Extract VISITING DETAILS section  
        var visitingItems = extractSectionItems(content, '\\*{0,2}\\s*visiting\\s+details\\s*\\*{0,2}', null);
        
        // Only update if we found items in both sections
        if (introItems.length > 0 && visitingItems.length > 0) {
          // Join items with line breaks
          var introContent = introItems.join('\n');
          var visitingContent = visitingItems.join('\n');
          
          // Update column J with intro items
          var columnJ = drafterSheet.getRange(row, 10);
          columnJ.setValue(introContent);
          columnJ.setBackground(CONFIG.COLORS.LIGHT_GREEN);
          
          // Update column I with visiting items (replacing existing content)
          var columnI = drafterSheet.getRange(row, 9);
          columnI.setValue(visitingContent);
          columnI.setBackground(CONFIG.COLORS.LIGHT_GREEN);
          
          // Update status in column Q to "Visit Formatted"
          drafterSheet.getRange(row, 17).setValue('Visit Formatted');
          
          processedCount++;
          Logger.log('✅ PARSED VISIT DETAILS: Row ' + row + ' - Found ' + introItems.length + ' intro items and ' + visitingItems.length + ' visiting items');
        } else {
          errorCount++;
          Logger.log('❌ FAILED TO PARSE: Row ' + row + ' - Could not find valid items. Intro: ' + introItems.length + ', Visiting: ' + visitingItems.length);
        }
      }
    } catch (error) {
      errorCount++;
...
function showVisitDetailsResultsAndAskNext(result, operationType) {
  var resultMessage = '🚙 PARSE VISIT DETAILS COMPLETE!\n\n';
  resultMessage += '👤 Workspace: ' + result.workspaceName + '\n';
  resultMessage += '✅ Successfully processed: ' + result.processed + ' articles\n';
  ...
  if (result.processed > 0) {
    resultMessage += '\n🎉 Visit details successfully parsed!\n';
    resultMessage += '📌 Intro details in column J with green background.\n';
    resultMessage += '📌 Visit details in column I with green background.\n';
    resultMessage += '📌 Status updated to "Visit Formatted".\n';
  }
...
function askForNextVisitDetailsWorkspace(operationType) {
  var response = SpreadsheetApp.getUi().alert(
    'No Articles Found',
    'Would you like to try another workspace?',
    SpreadsheetApp.getUi().ButtonSet.YES_NO
...
function batchFormatDraftDocuments() {
  var operationType = 'Batch Format Draft Documents';
  ...
  // Start the workspace selection loop
  processFormatDocsWorkspaceLoop(operationType);
}

function processFormatDocsWorkspaceLoop(operationType) {
  try {
    // Step 1: Ask user to choose workspace
    var selectedWorkspace = selectSingleWorkspaceForDrafterBasic('Format Draft Documents'); 
    ...
    if (selectedWorkspace.isAllWorkspaces) {
      ...
      if (!selectedWorkspace) {
        SpreadsheetApp.getUi().alert('Could not find workspace boundaries for the selected person.');
        askForNextFormatDocsWorkspace(operationType);
        return;
      }
    }
    ...
    if (selectedWorkspace.isAllWorkspaces) {
      ...
      for (var i = 0; i < selectedWorkspace.workspaces.length; i++) {
        var workspace = selectedWorkspace.workspaces[i];
        var workspaceArticles = getArticlesForFormatDocs(drafterSheet, workspace);
        articlesInWorkspace = articlesInWorkspace.concat(workspaceArticles);
      }
    } else {
      articlesInWorkspace = getArticlesForFormatDocs(drafterSheet, selectedWorkspace);
    }
    
    if (articlesInWorkspace.length === 0) {
      SpreadsheetApp.getUi().alert('No articles found with "Add ###" status and valid Google Doc URLs in the selected workspace(s).');
      askForNextFormatDocsWorkspace(operationType);
      return;
    }
    
    // Step 4: Show plan and process
    if (!showFormatDocsPlan(selectedWorkspace, articlesInWorkspace)) {
      askForNextFormatDocsWorkspace(operationType);
      return;
    }
    
    var result = processFormatDocsWorkspace(selectedWorkspace, articlesInWorkspace);
    showFormatDocsResultsAndAskNext(result, operationType);
    
  } catch (error) {
    Logger.log('Error in processFormatDocsWorkspaceLoop: ' + error.message);
    unlockUploaderSheet(operationType);
    SpreadsheetApp.getUi().alert('Error: ' + error.message);
  }
}

// Helper functions for Format Docs
function getArticlesForFormatDocs(drafterSheet, workspace) {
  var articles = [];
  var endRow = workspace.endRow || drafterSheet.getLastRow();
  
  Logger.log('🔍 SCANNING WORKSPACE: ' + workspace.personName + ' for Format Docs');
  
  for (var row = workspace.startRow; row <= endRow; row++) {
    var statusQ = drafterSheet.getRange(row, 17).getValue(); // Column Q - Status
    var docUrl = drafterSheet.getRange(row, 16).getValue(); // Column P - Doc URL (FIXED)
    
    if (statusQ === 'Add ###' && docUrl && docUrl.toString().includes('docs.google.com')) {
      articles.push({
        row: row,
        statusQ: statusQ,
        docUrl: docUrl,
      ...
      Logger.log('📝 FOUND FORMAT DOC: Row ' + row + ' (Status: ' + statusQ + ')');
    }
  }
...
function showFormatDocsPlan(workspace, articles) {
  var message = '📝 FORMAT DRAFT DOCUMENTS PLAN\n\n';
  ...
  if (workspace.isAllWorkspaces) {
    ...
    message += '👤 WORKSPACE: ' + workspace.personName + '\n';
    message += '📊 Documents to format: ' + articles.length + '\n\n';
    
    message += '📋 DOCUMENTS FOUND (Status: "Add ###"):\n';
    for (var i = 0; i < Math.min(articles.length, 8); i++) {
      message += '• Row ' + articles[i].row + '\n';
    }
    
    if (articles.length > 8) {
      message += '... and ' + (articles.length - 8) + ' more documents\n';
    }
  }
  
  message += '\n🔄 PROCESSING:\n';
  message += '• Format ## headings as H2 (bold)\n';
  message += '• Add "### Paste URL Here" after each H2\n';
  message += '• Style ### in dark green, highlight "Paste URL Here" in yellow\n';
  message += '• Skip already formatted documents\n';
  message += '• Update status to "For Transfer"\n\n';
  
  message += 'Continue with this workspace?';
  
  var response = SpreadsheetApp.getUi().alert(
    'Format Documents Plan',
    message,
    SpreadsheetApp.getUi().ButtonSet.YES_NO
  );
...
function processFormatDocsWorkspace(workspace, articles) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var drafterSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
  var processedCount = 0;
  var errorCount = 0;
  var alreadyFormattedCount = 0;
  
  Logger.log('📝 PROCESSING WORKSPACE: ' + workspace.personName + ' (' + articles.length + ' documents)');
  
  if (workspace.isAllWorkspaces) {
    for (var i = 0; i < workspace.workspaces.length; i++) {
      var singleWorkspace = workspace.workspaces[i];
      var workspaceArticles = getArticlesForFormatDocs(drafterSheet, singleWorkspace);
      
      if (workspaceArticles.length > 0) {
        Logger.log('📝 PROCESSING: ' + singleWorkspace.personName + ' (' + workspaceArticles.length + ' documents)');
        var workspaceResult = processFormatDocsArticles(drafterSheet, workspaceArticles);
        processedCount += workspaceResult.processed;
        errorCount += workspaceResult.errors;
        alreadyFormattedCount += workspaceResult.alreadyFormatted;
      }
    }
  } else {
    var result = processFormatDocsArticles(drafterSheet, articles);
    processedCount = result.processed;
    errorCount = result.errors;
...
function processFormatDocsArticles(drafterSheet, articles) {
  var processedCount = 0;
  var errorCount = 0;
  var alreadyFormattedCount = 0;
  ...
  for (var i = 0; i < articles.length; i++) {
    ...
    try {
      ...
      var docIdMatch = article.docUrl.match(/\/d\/([a-zA-Z0-9-_]+)/);
      if (!docIdMatch) {
        Logger.log('❌ Could not extract doc ID from URL in row ' + row);
        errorCount++;
        continue;
      }
      ...
      if (isDocumentAlreadyFormatted(doc)) {
        alreadyFormattedCount++;
        
        // Still update status to For Transfer since it might have been formatted manually
drafterSheet.getRange(row, 17).setValue('For Transfer');
        Logger.log('✅ Document already formatted, updating status: Row ' + row);
        continue;
      }
      
      // Format the document
      formatDocumentHeadings(doc);
...
drafterSheet.getRange(row, 17).setValue('For Transfer');
      ...
      processedCount++;
      Logger.log('✅ Formatted document in row ' + row);
...
function showFormatDocsResultsAndAskNext(result, operationType) {
  var resultMessage = '📝 FORMAT DRAFT DOCUMENTS COMPLETE!\n\n';
  resultMessage += '👤 Workspace: ' + result.workspaceName + '\n';
  resultMessage += '✅ Documents formatted: ' + result.processed + '\n';
  resultMessage += '📝 Already formatted (status updated): ' + result.alreadyFormatted + '\n';
  
  if (result.errors > 0) {
    resultMessage += '❌ Errors: ' + result.errors + '\n';
  }
  
  if (result.processed > 0 || result.alreadyFormatted > 0) {
    resultMessage += '\n🎉 Documents successfully processed!\n';
    resultMessage += '📌 All processed documents now have "For Transfer" status.\n';
    resultMessage += '📌 H2 headings formatted with ### URL placeholders.\n';
  }
...
function askForNextFormatDocsWorkspace(operationType) {
  var response = SpreadsheetApp.getUi().alert(
    'No Articles Found',
    'Would you like to try another workspace?',
    SpreadsheetApp.getUi().ButtonSet.YES_NO
...
// =============================================================================
// SHARED HELPER FUNCTIONS FOR DRAFTER BASIC WORKSPACE DETECTION
// =============================================================================

function selectSingleWorkspaceForDrafterBasic(operationName) {
  // HARDCODED workspace names for speed (same pattern as other functions)
  var hardcodedWorkspaces = [
    'JAMIE',
    'CHARL',
    'LARA',
  ...
  var ui = SpreadsheetApp.getUi();
  var response = ui.prompt(
    'Choose Workspace for ' + operationName,
    'Select YOUR workspace to process:\n\n' + 
    workspaceList + '\n' +
    'Enter a single number:',
    ui.ButtonSet.OK_CANCEL
  );
  ...
  if (choice === 0) {
    ...
    ui.alert('Invalid selection. Please enter a valid number.');
    return selectSingleWorkspaceForDrafterBasic(operationName); // Try again
  }
}

function findDrafterWorkspaceBoundaries(drafterSheet, personName) {
  var lastRow = drafterSheet.getLastRow();
  var startRow = null;
  var endRow = null;
  
  Logger.log('🔍 Finding Drafter Basic boundaries for: "' + personName + '"');
  
  try {
    // Scan column R (18) for the person's name
    for (var row = 2; row <= lastRow; row++) { // Start from row 2
      var cellR = drafterSheet.getRange(row, 18).getValue(); // Column R
      
      if (cellR && cellR.toString().trim().toLowerCase().includes(personName.toLowerCase())) {
        if (!startRow) {
          startRow = row;
          Logger.log('📍 Found ' + personName + ' start at row ' + row);
        }
        endRow = row; // Keep updating end row as we find more matches
      }
    }
    
    if (!startRow) {
      Logger.log('❌ Could not find "' + personName + '" in column R');
      return null;
    }
    ...
    Logger.log('✅ Drafter workspace boundaries for ' + personName + ': rows ' + startRow + ' to ' + (endRow || lastRow));
    return result;
    
  } catch (error) {
    Logger.log('❌ ERROR in findDrafterWorkspaceBoundaries for "' + personName + '": ' + error.message);
    return null;
  }
}
...
function checkStatusThenRecord() {
  var ui = SpreadsheetApp.getUi();
  
  var response = ui.alert(
    'Check Status & Record',
    'This will:\n1. Check WordPress status for all posts\n2. Record all published/scheduled posts\n\nThis may take several minutes. Continue?',
    ui.ButtonSet.YES_NO
  );
...
function recordAllProductionTracker() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEETS.WIYS_PRODUCTION_TRACKER);
  if (!sheet) {
    SpreadsheetApp.getUi().alert('WIYS Production Tracker sheet not found');
    return;
  }
  ...
  for (var row = 2; row <= lastRow; row++) {
    var cellEValue = sheet.getRange(row, 5).getValue(); // Column E (website URL)
    var currentStatus = sheet.getRange(row, 7).getValue(); // Column G (status)
    
    if (cellEValue && cellEValue.toString().includes('wheninyourstate.com')) {
      
      if (!cellEValue.toString().includes('http') ||
          currentStatus === 'MSN Pending' ||
          currentStatus === CONFIG.STATUS.DONE) {
      ...
      try {
        var postId = extractPostIdFromUrl(cellEValue.toString());
        if (!postId) {
          Logger.log('Could not extract post ID from URL in row ' + row + ': ' + cellEValue);
          errorCount++;
          continue;
        }
        ...
        if (wpStatus && wpStatus.status === CONFIG.WP_POST_STATUS.PUBLISH) {
          ...
          sheet.getRange(row, 3).setValue(publishDate);     // Column C (date/time)
          sheet.getRange(row, 4).setValue(cleanTitle);      // Column D (title)
          sheet.getRange(row, 5).setValue(wpStatus.link);   // Column E (website URL)
          sheet.getRange(row, 7).setValue("MSN Pending");   // Column G (status)
          
          processedCount++;
          Logger.log('Processed row ' + row + ' with published article: ' + cleanTitle);
          
          Utilities.sleep(500);
          
        } else if (wpStatus) {
          Logger.log('Row ' + row + ' - Post is not published yet. Status: ' + wpStatus.status);
        } else {
          Logger.log('Row ' + row + ' - Could not retrieve post data');
          errorCount++;
        }
  ...
  SpreadsheetApp.getUi().alert(
    'Production Tracker Processing Complete\n\n' +
    'Published posts processed: ' + processedCount + '\n' +
    'Already processed/skipped: ' + alreadyProcessedCount + '\n' +
    'Errors: ' + errorCount
  );
}
...
function preloadStateCategoryCache() {
  var states = [
  ...
  Logger.log('Pre-loading state category cache...');
  for (var i = 0; i < states.length; i++) {
    getCachedCategoryId(states[i]);
    Utilities.sleep(200); // Small delay
  }
  Logger.log('State category cache pre-loaded!');
}



function preloadAllWordPressTagsOnce() {
  var username = CONFIG.WORDPRESS.USERNAME;
  var applicationPassword = CONFIG.WORDPRESS.APP_PASSWORD;
  var cache = PropertiesService.getScriptProperties();
  var tagsEndpoint = CONFIG.WORDPRESS.BASE_URL + "/wp-json/wp/v2/tags";
  ...
  var ui = SpreadsheetApp.getUi();
  ui.alert('⏳ Starting Tag Preload', 'Loading all existing WordPress tags into cache...\n\nThis will take 2-3 minutes.\nDo not close this tab!', ui.ButtonSet.OK);
  
  Logger.log('🏷️ Starting one-time preload of all existing WordPress tags...');
  
  var page = 1;
  var totalTagsCached = 0;
  
  while (true) {
    var requestUrl = tagsEndpoint + "?per_page=100&page=" + page;
    ...
    try {
      ...
      if (response.getResponseCode() !== CONFIG.HTTP_STATUS.OK) {
        Logger.log('❌ Failed to fetch tags page ' + page + ': ' + response.getResponseCode());
        break;
      }
      ...
      if (tags.length === 0) {
        Logger.log('✅ No more tags found. Preloading complete.');
        break;
      }
      ...
      for (var i = 0; i < tags.length; i++) {
        var tag = tags[i];
        var cacheKey = 'tag_' + tag.name.toLowerCase().replace(/[^a-z0-9]/g, '_');
        cache.setProperty(cacheKey, tag.id.toString());
        totalTagsCached++;
      }
      
      Logger.log('📋 Cached page ' + page + ': ' + tags.length + ' tags (Total: ' + totalTagsCached + ')');
      
      page++;
      Utilities.sleep(500); // Small delay
      
    } catch (error) {
      Logger.log('❌ Error fetching tags page ' + page + ': ' + error.message);
      break;
    }
  }
  
  // Mark today as refreshed so daily refresh doesn't run immediately
  var today = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  cache.setProperty('last_tag_refresh_date', today);
  
  Logger.log('🎉 One-time preload complete! Cached ' + totalTagsCached + ' existing tags.');
  
  ui.alert(
    '🎉 Tag Preload Complete!', 
    'Successfully cached ' + totalTagsCached + ' existing WordPress tags.\n\n' +
    '✅ Your uploads will now be much faster!\n' +
    '✅ Cache will auto-refresh daily at 7 AM.\n' +
...
// Function to check if document is already formatted
function isDocumentAlreadyFormatted(doc) {
  var body = doc.getBody();
  var text = body.getText();
  
  // Check if document contains our URL placeholder
  return text.includes('### Paste URL Here');
}

// Function to format document headings
function formatDocumentHeadings(doc) {
  var body = doc.getBody();
  var totalElements = body.getNumChildren();
  
  // Process in reverse to handle insertions properly
  for (var i = totalElements - 1; i >= 0; i--) {
    ...
    if (element.getType() === DocumentApp.ElementType.PARAGRAPH) {
      ...
      // Skip if this is an H1 (exclude H1s as requested)
      if (heading === DocumentApp.ParagraphHeading.HEADING1) {
        continue;
      }
      
      // Check if this paragraph starts with ## (H2)
      if (text.match(/^##\s+(.+)/)) {
        // Format as H2
        ...
        // Check if we should add URL placeholder
        var nextElementIndex = i + 1;
        var shouldAddUrl = true;
        
        if (nextElementIndex < body.getNumChildren()) {
          ...
          if (nextElement.getType() === DocumentApp.ElementType.PARAGRAPH) {
            ...
            // Don't add if next paragraph already starts with ###
            if (nextText.match(/^###\s+/)) {
              shouldAddUrl = false;
            }
          }
        ...
        if (shouldAddUrl) {
          // Insert URL placeholder paragraph
          var urlParagraph = body.insertParagraph(nextElementIndex, '### Paste URL Here');
          
          // Format as H3
          urlParagraph.setHeading(DocumentApp.ParagraphHeading.HEADING3);
          
          // Style the URL placeholder
          var urlText = urlParagraph.editAsText();
          
          // Make entire text bold
          urlText.setBold(true);
          
          // Set font size to 13
          urlText.setFontSize(13);
          
          // Format the ### in dark green 2 (#0b5e3f)
          urlText.setForegroundColor(0, 2, CONFIG.COLORS.DARK_GREEN);
          
          // Highlight "Paste URL Here" in bright yellow
          urlText.setBackgroundColor(4, 17, CONFIG.COLORS.YELLOW);
        }
      }
    }
  }
...
var ss = SpreadsheetApp.getActiveSpreadsheet();
var stateTopicsSheet = ss.getSheetByName(CONFIG.SHEETS.STATE_TOPICS);
var topicPlanningSheet = ss.getSheetByName(CONFIG.SHEETS.TOPIC_PLANNING); // Changed from 'Drafter Basic'

// Check if both sheets exist
if (!stateTopicsSheet) {
  SpreadsheetApp.getUi().alert('Error', 
                              'No sheet named "State Topics" found. Please create it first.', 
                              SpreadsheetApp.getUi().ButtonSet.OK);
  return;
}

if (!topicPlanningSheet) { // Updated variable name and sheet name
  SpreadsheetApp.getUi().alert('Error', 
                              'No sheet named "Topic Planning" found.', // Updated message
                              SpreadsheetApp.getUi().ButtonSet.OK);
  return;
}

// Ask user how many sets they want to collect
var ui = SpreadsheetApp.getUi();
var response = ui.prompt('Collect Topics', 'How many sets do you want to collect?', ui.ButtonSet.OK_CANCEL);

if (response.getSelectedButton() == ui.Button.CANCEL) {
  return;
}

var numberOfSets = parseInt(response.getResponseText());

if (isNaN(numberOfSets) || numberOfSets <= 0) {
  ui.alert('Error', 'Please enter a valid number greater than 0.', ui.ButtonSet.OK);
  return;
}

// Array of states in the same order as columns (starting from column B)
var states = [
  'Alabama', 'Alaska', 'Arizona', 'Arkansas', 'California', 'Colorado', 
  'Connecticut', 'Delaware', 'Florida', 'Georgia', 'Hawaii', 'Idaho', 
  'Illinois', 'Indiana', 'Iowa', 'Kansas', 'Kentucky', 'Louisiana', 
...
// STEP 1: READ ALL DATA AT ONCE from State Topics
var startRow = 3; // Changed from 4 to 3 (skip rows 1 and 2 which are headers)
var endRow = startRow + numberOfSets - 1;
var startCol = 2; // Column B
var endCol = startCol + states.length - 1;

// Batch read all the data we need
var dataRange = stateTopicsSheet.getRange(startRow, startCol, numberOfSets, states.length);
var allData = dataRange.getValues();

// STEP 2: PROCESS DATA IN MEMORY
var allCollectedTopics = [];
var allWikimediaUrls = [];
var allStateNames = [];
var allShutterstockUrls = [];
var totalTopicsCollected = 0;
var processedSets = 0;

// Process each set (row)
for (var setIndex = 0; setIndex < numberOfSets; setIndex++) {
  var rowData = allData[setIndex];
  var topicsInThisSet = 0;
  
  // Process each state (column) in this set
  for (var stateIndex = 0; stateIndex < states.length; stateIndex++) {
    var topicValue = rowData[stateIndex];
    
    if (topicValue && topicValue.toString().trim() !== '') {
      var topic = topicValue.toString().trim();
      var topicWithState = topic + ', ' + states[stateIndex];
      
      // Generate Shutterstock URL
      var shutterstockUrl = generateShutterstockUrl(topicWithState);
      // Generate Wikimedia Commons URL
      var wikimediaUrl = generateWikimediaUrl(topicWithState);
      
      allCollectedTopics.push([topicWithState]); // Array format for setValues()
      allStateNames.push([states[stateIndex]]); // Just the state name for column E
      allShutterstockUrls.push([shutterstockUrl]); // Shutterstock URL for column G
      allWikimediaUrls.push([wikimediaUrl]); 
      topicsInThisSet++;
      totalTopicsCollected++;
    }
  }
  
  if (topicsInThisSet > 0) {
    processedSets++;
  } else {
    // Stop if we find a completely empty set
    break;
  }
}

// Check if we collected any topics
if (totalTopicsCollected === 0) {
  ui.alert('No Topics Found', 
           'No topics were found in the specified rows. Please add topics first.', 
           ui.ButtonSet.OK);
  return;
}

// STEP 3: PASTE TO TOPIC PLANNING
var startRowTopicPlanning = findNextEmptyRowInColumn(topicPlanningSheet, 4); // Updated to column D

// Batch write data to four columns
if (allCollectedTopics.length > 0) {
  // Write topics with state to column D
  var topicsRange = topicPlanningSheet.getRange(startRowTopicPlanning, 4, allCollectedTopics.length, 1);
  topicsRange.setValues(allCollectedTopics);
  
  // Write just state names to column E
  var statesRange = topicPlanningSheet.getRange(startRowTopicPlanning, 5, allStateNames.length, 1);
  statesRange.setValues(allStateNames);
  
  // Write Shutterstock URLs to column G
  var urlsRange = topicPlanningSheet.getRange(startRowTopicPlanning, 7, allShutterstockUrls.length, 1);
  urlsRange.setValues(allShutterstockUrls);
  
  // Write Wikimedia URLs to column H
  var wikimediaRange = topicPlanningSheet.getRange(startRowTopicPlanning, 8, allWikimediaUrls.length, 1);
  wikimediaRange.setValues(allWikimediaUrls);
  
  // Batch format all ranges
  topicsRange.setHorizontalAlignment('center').setVerticalAlignment('middle');
  statesRange.setHorizontalAlignment('center').setVerticalAlignment('middle');
  urlsRange.setHorizontalAlignment('center').setVerticalAlignment('middle');
  wikimediaRange.setHorizontalAlignment('center').setVerticalAlignment('middle');
}

// STEP 4: CLEAR AND DELETE ROWS FROM STATE TOPICS
// First, clear the content from the rows we used
dataRange.clearContent();

// Then delete the empty rows (delete from bottom to top to avoid index shifting)
for (var rowToDelete = endRow; rowToDelete >= startRow; rowToDelete--) {
  stateTopicsSheet.deleteRow(rowToDelete);
}

// STEP 5: Show success message
var message = '✅ Collect Topics Complete!\n\n';
message += '📊 SUMMARY:\n';
message += '• Sets requested: ' + numberOfSets + '\n';
message += '• Sets processed: ' + processedSets + '\n';
message += '• Total topics collected: ' + totalTopicsCollected + '\n';
message += '• Rows deleted from State Topics: ' + processedSets + '\n';
message += '• Placed in Topic Planning starting at row ' + startRowTopicPlanning + '\n';
message += '• Column D: Topics with state names\n';
message += '• Column E: State names only\n';
message += '• Column G: Shutterstock search URLs\n';
message += '• Column H: Wikimedia Commons search URLs\n';
message += '• All content centered\n\n';

if (processedSets < numberOfSets) {
  message += '⚠️ NOTE: Only ' + processedSets + ' sets were processed (some rows were empty)\n\n';
}

message += '📋 SAMPLE COLLECTED TOPICS:\n';
var sampleCount = Math.min(3, allCollectedTopics.length);
for (var m = 0; m < sampleCount; m++) {
  message += '• ' + allCollectedTopics[m][0] + ' | ' + allStateNames[m][0] + '\n';
}

if (allCollectedTopics.length > 3) {
  message += '... and ' + (allCollectedTopics.length - 3) + ' more topics';
}

message += '\n🔄 Topics moved to Topic Planning with search URLs and source rows deleted!';

ui.alert('Success!', message, ui.ButtonSet.OK);
}

// Helper function to generate Shutterstock search URL
function generateShutterstockUrl(placeName) {
  // Convert place name to URL-encoded format
  var encodedPlaceName = encodeURIComponent(placeName.toLowerCase());
  
  // Build the Shutterstock search URL
  var shutterstockUrl = 'https://www.shutterstock.com/search/' + encodedPlaceName + '?image_type=photo';
  
  return shutterstockUrl;
}

// Helper function to generate Wikimedia Commons search URL
function generateWikimediaUrl(placeName) {
  // Convert place name to URL-encoded format for search
  var encodedPlaceName = encodeURIComponent(placeName);
  
  // Build the Wikimedia Commons MediaSearch URL
  var wikimediaUrl = 'https://commons.wikimedia.org/wiki/Special:MediaSearch?search=' + encodedPlaceName + '&type=image';
  
  return wikimediaUrl;
}

// Helper function to find the next empty row in a specific column (starting from row 10 minimum)
function findNextEmptyRowInColumn(sheet, columnIndex) {
var lastRow = sheet.getLastRow();
var startFromRow = 3;

// Batch read the entire column from row 10 onwards
if (lastRow >= startFromRow) {
  var checkRange = sheet.getRange(startFromRow, columnIndex, lastRow - startFromRow + 1, 1);
  var values = checkRange.getValues();
  
  // Find first empty cell
  for (var i = 0; i < values.length; i++) {
    if (!values[i][0] || values[i][0].toString().trim() === '') {
      return startFromRow + i;
    }
  }
}

// If no empty rows found, return the row after the last used row
return Math.max(lastRow + 1, startFromRow);
}

function topicsToCells() {
// Get the State Topics sheet
var ss = SpreadsheetApp.getActiveSpreadsheet();
var sheet = ss.getSheetByName(CONFIG.SHEETS.STATE_TOPICS);
...
// Array of states in the same order as your columns (starting from column B)
var states = [
 'Alabama', 'Alaska', 'Arizona', 'Arkansas', 'California', 'Colorado', 
 'Connecticut', 'Delaware', 'Florida', 'Georgia', 'Hawaii', 'Idaho', 
 'Illinois', 'Indiana', 'Iowa', 'Kansas', 'Kentucky', 'Louisiana', 
...
// Check each state column (starting from column B = index 2)
for (var i = 0; i < states.length; i++) {
 var stateName = states[i];
 var columnIndex = i + 2; // Column B is index 2, C is 3, etc.
 
 // Get the content from row 2 for this state
 var listContent = sheet.getRange(2, columnIndex).getValue();
 
 if (listContent && listContent.toString().trim() !== '') {
   Logger.log('Processing ' + stateName + ' in column ' + columnIndex);
   
   // Parse the list content using the enhanced parser
   var items = parseListContent(listContent.toString(), stateName);
   
   if (items.length > 0) {
     // Find the last used row in this column
     var lastRow = findLastUsedRowInColumn(sheet, columnIndex);
     
     // Start pasting from the next row
     var startRow = lastRow + 1;
     
     // Paste each item in a new row AND CENTER IT (WITHOUT STATE NAME - that gets added later)
     for (var k = 0; k < items.length; k++) {
       var targetRow = startRow + k;
       var targetCell = sheet.getRange(targetRow, columnIndex);
       
       // Set the value WITHOUT appending state name (removed the duplicate)
       targetCell.setValue(items[k]);
       
       // Center the content horizontally and vertically
       targetCell.setHorizontalAlignment('center');
       targetCell.setVerticalAlignment('middle');
       
       Logger.log('Pasted & centered "' + items[k] + '" to ' + stateName + ' row ' + targetRow);
     }
     
     // Alternative: Batch centering (more efficient for large lists)
     if (items.length > 1) {
       var rangeToCenter = sheet.getRange(startRow, columnIndex, items.length, 1);
       rangeToCenter.setHorizontalAlignment('center');
       rangeToCenter.setVerticalAlignment('middle');
       Logger.log('Batch centered ' + items.length + ' items in ' + stateName);
     }
     
     // Clear the original row 2 content for this state
     sheet.getRange(2, columnIndex).clearContent();
     ...
     Logger.log('Completed ' + stateName + ': ' + items.length + ' items moved and centered');
   }
 }
}

// Show results
if (processedStates > 0) {
 var message = '✅ Topics to Cells Complete!\n\n';
 message += '📊 SUMMARY:\n';
 message += '• States processed: ' + processedStates + '\n';
 message += '• Total topics moved: ' + totalItemsProcessed + '\n';
 message += '• All content centered horizontally & vertically\n';
 message += '• Topics ready for collection (state names will be added during collection)\n\n';
 message += '📋 DETAILS:\n';
 
 for (var m = 0; m < Math.min(processedDetails.length, 8); m++) {
   var detail = processedDetails[m];
   message += '• ' + detail.state + ': ' + detail.items + ' topics (starting row ' + detail.startRow + ')\n';
 }
 
 if (processedDetails.length > 8) {
   message += '... and ' + (processedDetails.length - 8) + ' more states\n';
 }
 
 message += '\n🧹 Row 2 lists have been cleared!';
 message += '\n🎯 All content is now centered!';
 
 SpreadsheetApp.getUi().alert('Success!', message, SpreadsheetApp.getUi().ButtonSet.OK);
} else {
 SpreadsheetApp.getUi().alert('No Data Found', 
                             'No topic lists found in row 2. Make sure you have lists in row 2 for the states you want to process.', 
                             SpreadsheetApp.getUi().ButtonSet.OK);
}

Logger.log('✅ Topics to Cells completed - ' + processedStates + ' states, ' + totalItemsProcessed + ' total items, all centered');
}

// Enhanced function to parse different list formats
function parseListContent(content, stateName) {
var items = [];

// Clean up the content
content = content.trim();

// Remove any title/header lines (like "**ALABAMA: Complete Historic Sites...**")
// Look for lines that contain the state name and title-like words
var lines = content.split('\n');
var cleanedContent = '';
...
if (lines.length > 0) {
 ...
 // Skip if it contains state name and title-like indicators
 if (firstLine.includes(upperStateName) || 
     firstLine.includes('**') || 
     firstLine.includes('Complete') || 
     firstLine.includes('List') ||
     firstLine.includes(':')) {
   skipFirstLine = true;
...
Logger.log('Cleaned content for ' + stateName + ': ' + cleanedContent.substring(0, 200) + '...');

// Try different parsing methods in order of preference

// Method 1: Split by bullet points (•)
if (cleanedContent.includes('•')) {
 var bulletItems = cleanedContent.split('•');
 for (var i = 0; i < bulletItems.length; i++) {
   var item = bulletItems[i].trim();
   if (item !== '') {
     items.push(item);
   }
 }
 Logger.log('Parsed using bullet points (•): ' + items.length + ' items');
 return items;
}

// Method 2: Split by line breaks (handles numbered lists, dashed lists, etc.)
var lineItems = cleanedContent.split('\n');
var hasValidLineItems = false;

for (var j = 0; j < lineItems.length; j++) {
 var lineItem = lineItems[j].trim();
 
 // Clean up common list prefixes
 lineItem = cleanupListItem(lineItem);
...
if (hasValidLineItems && items.length > 1) {
 Logger.log('Parsed using line breaks: ' + items.length + ' items');
 return items;
}

// Method 3: Split by common separators (commas, semicolons)
items = []; // Reset
var separators = [';', ','];

for (var s = 0; s < separators.length; s++) {
 var separator = separators[s];
 if (cleanedContent.includes(separator)) {
   var sepItems = cleanedContent.split(separator);
   items = [];
   
   for (var k = 0; k < sepItems.length; k++) {
     var sepItem = sepItems[k].trim();
     sepItem = cleanupListItem(sepItem);
   ...
   if (items.length > 1) {
     Logger.log('Parsed using separator "' + separator + '": ' + items.length + ' items');
     return items;
   }
 }
}

// Method 4: If all else fails, treat as single item
if (cleanedContent !== '') {
 items = [cleanedContent];
 Logger.log('Treated as single item: ' + cleanedContent.substring(0, 50) + '...');
}

return items;
}

// Helper function to clean up individual list items
function cleanupListItem(item) {
...
// Remove dash prefixes (-, --, etc.)
item = item.replace(/^-+\s*/, '');

// Remove bullet prefixes that might have been missed
item = item.replace(/^[•·▪▫◦‣⁃]\s*/, '');

// Remove asterisk prefixes
item = item.replace(/^\*+\s*/, '');

// Remove parentheses prefixes like (a), (i), etc.
item = item.replace(/^\([a-zA-Z0-9]+\)\s*/, '');
...
function findLastUsedRowInColumn(sheet, columnIndex) {
 var lastRow = sheet.getLastRow();
 
 // If sheet is empty or we're starting from the top
 if (lastRow < 3) {
   return 3; // Start from row 4 (3 + 1)
 }
 
 // Check from the bottom up to find the last non-empty cell in this column
 ...
 // If no content found in this column, start from row 3
 return 3;
}
...
function divideTopicDefiner() {
 ...
 // Get the content from column A, row 2
 var listContent = sheet.getRange(2, 1).getValue(); // Column A = index 1
 
 if (!listContent || listContent.toString().trim() === '') {
   SpreadsheetApp.getUi().alert('No Data Found', 
                               'No URL list found in cell A2. Please paste your URLs there first.', 
                               SpreadsheetApp.getUi().ButtonSet.OK);
   return;
 }
 
 Logger.log('Processing URL list in column A, row 2');
 
 // Parse the URL list content
 var urls = parseUrlList(listContent.toString());
 
 if (urls.length === 0) {
   SpreadsheetApp.getUi().alert('No URLs Found', 
                               'No valid URLs could be extracted from the content in A2.', 
                               SpreadsheetApp.getUi().ButtonSet.OK);
   return;
 }
 
 // Find the last used row in column A
 var lastRow = findLastUsedRowInColumn(sheet, 1); // Column A = index 1
 
 // Start pasting from row 3 (or next available row if content exists below row 2)
 var startRow = Math.max(3, lastRow + 1);
 
 // Paste each URL in a new row in column A
 for (var i = 0; i < urls.length; i++) {
   var targetRow = startRow + i;
   var targetCell = sheet.getRange(targetRow, 1); // Column A
   
   // Set the URL value
   targetCell.setValue(urls[i]);
   
   // Optional: Center the content (remove these lines if you don't want centering)
   targetCell.setHorizontalAlignment('center');
   targetCell.setVerticalAlignment('middle');
   
   Logger.log('Pasted URL "' + urls[i] + '" to row ' + targetRow);
 }
 
 // Clear the original row 2 content
 sheet.getRange(2, 1).clearContent();
 
 // Show success message
 var message = '✅ Divide Topic Definer Complete!\n\n';
 message += '📊 SUMMARY:\n';
 message += '• URLs processed: ' + urls.length + '\n';
 message += '• Starting row: ' + startRow + '\n';
 message += '• Ending row: ' + (startRow + urls.length - 1) + '\n\n';
 message += '🧹 Original A2 content has been cleared!\n';
 message += '🎯 URLs are now in separate cells in column A!';
 
 SpreadsheetApp.getUi().alert('Success!', message, SpreadsheetApp.getUi().ButtonSet.OK);
 
 Logger.log('✅ Divide Topic Definer completed - ' + urls.length + ' URLs processed');
}

// Function to parse URL lists from various formats
function parseUrlList(content) {
 ...
 Logger.log('Parsing URL content: ' + content.substring(0, 200) + '...');
 
 // Method 1: Split by line breaks (most common for URL lists)
 var lines = content.split('\n');
 ...
 for (var i = 0; i < lines.length; i++) {
   ...
   // Clean up common list prefixes
   line = cleanupListItem(line);
   
   // Check if it looks like a URL (basic validation)
   if (line !== '' && isValidUrl(line)) {
     urls.push(line);
     hasValidUrls = true;
   } else if (line !== '' && !isValidUrl(line)) {
     // If it's not empty but doesn't look like a URL, still add it
     // (user might have non-standard URLs or want to include them anyway)
     urls.push(line);
   }
 }
 
 if (hasValidUrls || urls.length > 1) {
   Logger.log('Parsed using line breaks: ' + urls.length + ' URLs');
   return urls;
 }
 
 // Method 2: Split by spaces (if URLs are space-separated)
 urls = []; // Reset
 var spaceItems = content.split(/\s+/);
 
 for (var j = 0; j < spaceItems.length; j++) {
   var item = spaceItems[j].trim();
   if (item !== '') {
     urls.push(item);
   }
 }
 ...
 if (urls.length > 1) {
   Logger.log('Parsed using spaces: ' + urls.length + ' URLs');
   return urls;
 }
 
 // Method 3: Split by commas (if URLs are comma-separated)
 urls = []; // Reset
 if (content.includes(',')) {
   var commaItems = content.split(',');
   
   for (var k = 0; k < commaItems.length; k++) {
     var commaItem = commaItems[k].trim();
     if (commaItem !== '') {
       urls.push(commaItem);
     }
   }
   ...
   if (urls.length > 1) {
     Logger.log('Parsed using commas: ' + urls.length + ' URLs');
     return urls;
   }
 }
 
 // Method 4: Treat as single URL
 if (content !== '') {
   urls = [content];
   Logger.log('Treated as single URL: ' + content.substring(0, 50) + '...');
 }
...
function isValidUrl(string) {
 ...
 var urlPattern = /^(https?:\/\/|www\.|[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9]*\.)/i;
 return urlPattern.test(string.trim());
}

// Helper function to find last used row in a specific column
function findLastUsedRowInColumn(sheet, columnIndex) {
 var values = sheet.getRange(1, columnIndex, sheet.getLastRow(), 1).getValues();
 
 for (var i = values.length - 1; i >= 0; i--) {
   if (values[i][0] !== null && values[i][0] !== undefined && values[i][0].toString().trim() !== '') {
...
/// ASSIGN CLAUDE LINKS AND PLACES


function assignTopics() {
  // Get the Topic Planning sheet
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(CONFIG.SHEETS.TOPIC_PLANNING);
  
  if (!sheet) {
    SpreadsheetApp.getUi().alert('Error', 
                                'No sheet named "Topic Planning" found. Please create it first.', 
                                SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  
  // Get the content from column F, row 2
  var listContent = sheet.getRange(2, 6).getValue(); // Column F = index 6
  
  if (!listContent || listContent.toString().trim() === '') {
    SpreadsheetApp.getUi().alert('No Data Found', 
                                'No URL list found in cell F2. Please paste your URLs there first.', 
                                SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  
  Logger.log('Processing URL list in column F, row 2');
  ...
  if (urls.length === 0) {
    SpreadsheetApp.getUi().alert('No URLs Found', 
                                'No valid URLs could be extracted from the content in F2.', 
                                SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  
  // Show numbered list to choose assignee
  var ui = SpreadsheetApp.getUi();
  var namesList = ['Charl', 'Lara', 'Shayne', 'Naintara', 'Karl', 'Marie', 'Jamie'];
  
  // Create numbered list for display
  var numberedList = '';
  for (var i = 0; i < namesList.length; i++) {
    numberedList += (i + 1) + '. ' + namesList[i] + '\n';
  }
  
  var response = ui.prompt('Choose Assignee', 
                          'Who should these ' + urls.length + ' URLs be assigned to?\n\n' + numberedList + '\nEnter number (1-' + namesList.length + '):', 
                          ui.ButtonSet.OK_CANCEL);
  ...
  var selectedNumber = parseInt(userInput);
  
  // Validate the number
  if (isNaN(selectedNumber) || selectedNumber < 1 || selectedNumber > namesList.length) {
    ui.alert('Invalid Selection', 
             'Please enter a number between 1 and ' + namesList.length, 
             ui.ButtonSet.OK);
    return;
  }
  
  var selectedName = namesList[selectedNumber - 1]; // Convert to 0-based index
  
  // Find the last used row in column F
  var lastRow = findLastUsedRowInColumn(sheet, 6); // Column F = index 6
  
  // Start pasting from row 3 (or next available row if content exists below row 2)
  var startRow = Math.max(3, lastRow + 1);
  
  // Paste each URL in column F and name in column C
  for (var i = 0; i < urls.length; i++) {
    var targetRow = startRow + i;
    
    // Set the URL value in column F
    var urlCell = sheet.getRange(targetRow, 6); // Column F
    urlCell.setValue(urls[i]);
    urlCell.setHorizontalAlignment('center');
    urlCell.setVerticalAlignment('middle');
    
    // Set the assignee name in column C
    var nameCell = sheet.getRange(targetRow, 3); // Column C
    nameCell.setValue(selectedName);
    nameCell.setHorizontalAlignment('center');
    nameCell.setVerticalAlignment('middle');
    
    Logger.log('Assigned URL "' + urls[i] + '" to ' + selectedName + ' in row ' + targetRow);
  }
  
  // Clear the original row 2 content
  sheet.getRange(2, 6).clearContent(); // Clear F2
  
  // Show success message
  var message = '✅ Topic Assignment Complete!\n\n';
  message += '📊 SUMMARY:\n';
  message += '• URLs processed: ' + urls.length + '\n';
  message += '• Assigned to: ' + selectedName + '\n';
  message += '• Starting row: ' + startRow + '\n';
  message += '• Ending row: ' + (startRow + urls.length - 1) + '\n\n';
  message += '🧹 Original F2 content has been cleared!\n';
  message += '🎯 URLs are now in column F with assignments in column C!';
  
  SpreadsheetApp.getUi().alert('Success!', message, SpreadsheetApp.getUi().ButtonSet.OK);
  
  Logger.log('✅ Topic assignment completed - ' + urls.length + ' URLs assigned to ' + selectedName);
}



function transferToBasic() {
 var ss = SpreadsheetApp.getActiveSpreadsheet();
 var topicSheet = ss.getSheetByName(CONFIG.SHEETS.TOPIC_PLANNING);
 var basicSheet = ss.getSheetByName(CONFIG.SHEETS.DRAFTER_BASIC);
 var ui = SpreadsheetApp.getUi();
 
 // Check if both sheets exist
 if (!topicSheet) {
   ui.alert('Error', 'No sheet named "Topic Planning" found.', ui.ButtonSet.OK);
   return;
 }
 
 if (!basicSheet) {
   ui.alert('Error', 'No sheet named "Drafter Basic" found.', ui.ButtonSet.OK);
   return;
 }
 
 // Get all data from Topic Planning (starting from row 3)
 var topicLastRow = topicSheet.getLastRow();
 if (topicLastRow < 3) {
   ui.alert('No Data', 'No assigned topics found in Topic Planning sheet.', ui.ButtonSet.OK);
   return;
 }
 
 var topicData = topicSheet.getRange(3, 2, topicLastRow - 2, 8).getValues(); // B to I (columns 2-9)
 
 // Filter out incomplete rows and organize by assignee
 var assignments = {};
 var rowsToMarkDone = [];
 var incompleteRows = 0;
 
 for (var i = 0; i < topicData.length; i++) {
   var colB = topicData[i][0]; // Column B (index 0 in our range)
   var assignee = topicData[i][1]; // Column C (index 1 in our range)
   var colD = topicData[i][2]; // Column D (index 2 in our range)
   var colE = topicData[i][3]; // Column E (index 3 in our range)
   var colF = topicData[i][4]; // Column F (index 4 in our range)
   var colI = topicData[i][7]; // Column I (index 7 in our range)
   
   // Check if column B is "Yes" AND ALL columns C, D, E, F have content
   if (colB === "Yes" &&
       assignee && assignee.toString().trim() !== '' &&
       colD && colD.toString().trim() !== '' &&
       colE && colE.toString().trim() !== '' &&
       colF && colF.toString().trim() !== '') {
     
     if (!assignments[assignee]) {
       assignments[assignee] = [];
     }
     assignments[assignee].push({
       row: i + 3, // Actual row number in sheet
       colD: colD,
       colE: colE,
       colF: colF,
       colI: colI
     });
     rowsToMarkDone.push(i + 3);
   } else {
     // Count incomplete rows for reporting
     if (assignee || colD || colE || colF) {
       incompleteRows++;
     }
   }
 }
 ...
 if (Object.keys(assignments).length === 0) {
   var message = 'No valid assignments found in Topic Planning sheet.';
   if (incompleteRows > 0) {
     message += '\n\n' + incompleteRows + ' rows were skipped because they had incomplete data or column B was not "Yes".';
   }
   ui.alert('No Data', message, ui.ButtonSet.OK);
   return;
 }
 
 // Get all data from Drafter Basic to find empty rows for each person
 var basicLastRow = basicSheet.getLastRow();
 var basicData = basicSheet.getRange(1, 1, basicLastRow, 18).getValues(); // A to R
 
 var transferResults = {};
 var alertMessages = [];
 
 // Process each assignee
 for (var assignee in assignments) {
   var topics = assignments[assignee];
   var emptyRows = [];
   
   // Find empty rows for this assignee in Drafter Basic
   for (var j = 0; j < basicData.length; j++) {
     var basicAssignee = basicData[j][17]; // Column R
     var basicColB = basicData[j][1]; // Column B
     
     if (basicAssignee === assignee && (!basicColB || basicColB.toString().trim() === '')) {
       emptyRows.push(j + 1); // Actual row number
     }
   }
   
   // Check if we have enough empty rows
   if (emptyRows.length < topics.length) {
     alertMessages.push(assignee + ': Need ' + topics.length + ' empty rows, but only found ' + emptyRows.length);
     continue;
   }
   
   // Transfer the data
   var transferred = 0;
   for (var k = 0; k < topics.length && k < emptyRows.length; k++) {
     var targetRow = emptyRows[k];
     var topic = topics[k];
     
     // Set the values in Drafter Basic (D->B, E->C, F->D, I->N)
     basicSheet.getRange(targetRow, 2).setValue(topic.colD); // D -> B
     basicSheet.getRange(targetRow, 3).setValue(topic.colE); // E -> C
     basicSheet.getRange(targetRow, 4).setValue(topic.colF); // F -> D
     basicSheet.getRange(targetRow, 14).setValue(topic.colI); // I -> N (column 14)
     
     transferred++;
   }
   
   transferResults[assignee] = transferred;
 }
 
 // Show alerts for insufficient rows
 if (alertMessages.length > 0) {
   ui.alert('Insufficient Empty Rows', 
            'Some assignments could not be transferred:\n\n' + alertMessages.join('\n'), 
            ui.ButtonSet.OK);
   
   // Ask if user wants to continue with partial transfer
   var continueResponse = ui.alert('Continue?', 
                                  'Do you want to continue with partial transfer for other assignees?', 
                                  ui.ButtonSet.OK_CANCEL);
   if (continueResponse === ui.Button.CANCEL) {
     return;
   }
 }
 
 // Mark transferred rows as Done in column B (instead of deleting)
 if (Object.keys(transferResults).length > 0) {    
   for (var m = 0; m < rowsToMarkDone.length; m++) {
     var rowNum = rowsToMarkDone[m];
     // Only mark as Done if this assignee was successfully transferred
     var rowAssignee = topicSheet.getRange(rowNum, 3).getValue(); // Column C
     if (transferResults[rowAssignee] && transferResults[rowAssignee] > 0) {
       topicSheet.getRange(rowNum, 2).setValue('Done'); // Set column B to "Done"
     }
   }
 }
 
 // Show success message
 var successMsg = '✅ Transfer Complete!\n\n📊 SUMMARY:\n';
 var totalTransferred = 0;
 
 for (var assignee in transferResults) {
   successMsg += '• ' + assignee + ': ' + transferResults[assignee] + ' topics transferred\n';
   totalTransferred += transferResults[assignee];
 }
 
 successMsg += '\n✅ ' + totalTransferred + ' rows marked as Done in Topic Planning!';
 
 if (incompleteRows > 0) {
   successMsg += '\n\n⚠️ ' + incompleteRows + ' rows were skipped due to incomplete data or column B not being "Yes".';
 }
 
 if (alertMessages.length > 0) {
   successMsg += '\n\n⚠️ Some assignments could not be transferred due to insufficient empty rows.';
 }
 
 ui.alert('Success!', successMsg, ui.ButtonSet.OK);
 
 Logger.log('✅ Transfer completed - ' + totalTransferred + ' topics transferred and marked as Done, ' + incompleteRows + ' incomplete rows skipped');
}

function deleteNoImages() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(CONFIG.SHEETS.TOPIC_PLANNING);
  
  if (!sheet) {
    SpreadsheetApp.getUi().alert('Error', 
                                'No sheet named "Topic Planning" found.', 
                                SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  ...
  if (lastRow < 1) {
    ss.toast('No data found in the sheet', 'Nothing to Delete', 2);
    return;
  }
  
  // Read all values in column B at once (single API call)
  var columnBValues = sheet.getRange(1, 2, lastRow, 1).getValues();
  
  var deletedCount = 0;
  var clearedCount = 0;
  
  // First pass: Clear column F for "Yes" entries (top to bottom is fine for clearing)
  for (var i = 0; i < lastRow; i++) {
    var cellValue = columnBValues[i][0];
    
    if (cellValue === 'Yes') {
      sheet.getRange(i + 1, 6).clearContent(); // Clear column F (index 6)
      clearedCount++;
      Logger.log('Cleared column F in row ' + (i + 1) + ' with "Yes" in column B');
    }
  }
  
  // Second pass: Delete rows with "No" (bottom to top to avoid row shifting issues)
  for (var i = lastRow - 1; i >= 0; i--) {
    ...
    if (cellValue === 'No') {
      sheet.deleteRow(i + 1);
      deletedCount++;
      Logger.log('Deleted row ' + (i + 1) + ' with "No" in column B');
    }
  }
  ...
  if (deletedCount > 0 && clearedCount > 0) {
    message = deletedCount + ' row(s) deleted, ' + clearedCount + ' URL(s) cleared';
  } else if (deletedCount > 0) {
    message = deletedCount + ' row(s) with "No" deleted';
  } else if (clearedCount > 0) {
    message = clearedCount + ' URL(s) cleared from "Yes" rows';
  } else {
    message = 'No changes made - no "Yes" or "No" entries found';
  }
  
  ss.toast(message, 'Operation Complete', 3);
  Logger.log('✅ Operation completed - ' + deletedCount + ' rows deleted, ' + clearedCount + ' URLs cleared');
}
...
/**
 * Starts the article collection - fully automatic
 */
function populateArticleCollection() {
  var ui = SpreadsheetApp.getUi();

  var response = ui.alert(
    'Populate Article Collection',
    'This will fetch all published articles from July 16, 2025 to today.\n\n' +
    'It runs automatically in the background. You can close this and check back later.\n\n' +
    'Continue?',
    ui.ButtonSet.YES_NO
  );
  ...
  cleanupFetchTriggers();

  // Create the sheet
  createArticleCollectionSheet();

  // Set up properties
  var scriptProps = PropertiesService.getScriptProperties();
  scriptProps.setProperty('COLLECTION_END_DATE', '2025-07-16');
  scriptProps.setProperty('COLLECTION_CURRENT_DATE', new Date().toISOString().split('T')[0]);

  ui.alert('Started!', 'Collection is running in the background.\n\nYou can watch the sheet populate. A toast message will appear when complete.', ui.ButtonSet.OK);
...
/**
 * Removes all triggers for the collection function
 */
function cleanupFetchTriggers() {
  var triggers = ScriptApp.getProjectTriggers();
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'runCollectionBatch') {
...
/**
 * Runs a batch of fetching, then auto-schedules next batch
 */
function runCollectionBatch() {
  var scriptProps = PropertiesService.getScriptProperties();
  
  var currentDateStr = scriptProps.getProperty('COLLECTION_CURRENT_DATE');
  var endDateStr = scriptProps.getProperty('COLLECTION_END_DATE');
  
  if (!currentDateStr || !endDateStr) {
    Logger.log('No collection in progress');
    return;
  }
  ...
  var startTime = new Date();
  var maxRunTime = 5 * 60 * 1000; // 5 minutes
  var daysProcessed = 0;
  var articlesAdded = 0;
  
  while (true) {
    var currentDate = new Date(currentDateStr + 'T23:59:59');
    
    // Done?
    if (currentDate < endDate) {
      Logger.log('Collection complete!');
      SpreadsheetApp.getActiveSpreadsheet().toast('Article collection complete! All articles loaded.', 'Done!', 30);
      scriptProps.deleteProperty('COLLECTION_CURRENT_DATE');
      scriptProps.deleteProperty('COLLECTION_END_DATE');
      cleanupFetchTriggers();
      return;
    }
    ...
    if (new Date() - startTime > maxRunTime) {
      Logger.log('Batch done, scheduling next. Currently at: ' + currentDateStr);
      SpreadsheetApp.getActiveSpreadsheet().toast('Progress: ' + currentDateStr + ' | Added ' + articlesAdded + ' articles', 'Running...', 10);
      
      // Schedule next batch
      cleanupFetchTriggers();
      ScriptApp.newTrigger('runCollectionBatch')
        .timeBased()
        .after(30 * 1000)
        .create();
      return;
    ...
    // Fetch this day
    var dayStart = new Date(currentDateStr + 'T00:00:00');
    var dayEnd = new Date(currentDateStr + 'T23:59:59');
    
    Logger.log('Fetching: ' + currentDateStr);
    
    var articles = fetchArticlesForDateRange(username, appPassword, dayStart, dayEnd);
    
    if (articles.length > 0) {
      var categorizedArticles = recommendCategories(articles);
      appendArticlesToSheet(categorizedArticles);
    ...
    scriptProps.setProperty('COLLECTION_CURRENT_DATE', currentDateStr);
...
/**
 * Appends articles to the Collection sheet
 */
function appendArticlesToSheet(articles) {
 ...
 */
function stopArticleCollection() {
  var scriptProps = PropertiesService.getScriptProperties();
  scriptProps.deleteProperty('COLLECTION_CURRENT_DATE');
  scriptProps.deleteProperty('COLLECTION_END_DATE');
  cleanupFetchTriggers();
  
  SpreadsheetApp.getUi().alert('Stopped', 'Article collection stopped.', SpreadsheetApp.getUi().ButtonSet.OK);
}
...
/**
 * Writes articles to the Article Collection sheet
 */
function writeArticlesToCollectionSheet(sheet, articles) {
  if (articles.length === 0) return;
...
function setupDailyCollectionUpdate() {
  ...
  var triggers = ScriptApp.getProjectTriggers();
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'updateArticleCollection') {
      ScriptApp.deleteTrigger(triggers[i]);
    }
  }
...
function onOpen() {
  ...
  ui.createMenu('      *Editing')
    ...
    .addSeparator()
    .addSeparator()
    .addItem('🤔 Batch Check WP Status', 'checkAllWPStatuses')
    .addSeparator()
    .addItem('⏳ Set Date/Time - ALL', 'batchSetDateTime')
    ...
    .addItem('👉 Check WP Status + Record', 'checkStatusThenRecord')
    .addSeparator()
    .addItem('☑️ Finalize Titles/URLs', 'recordAllProductionTracker')
    .addToUi();
  ...
  ui.createMenu('      *Newsletter')
    .addItem('📋 Create Collection Sheet', 'createArticleCollectionSheet')
    .addItem('📥 Populate Article Collection', 'populateArticleCollection')
    ...
    .addSeparator()
    .addItem('⏰ Setup Daily Auto-Update', 'setupDailyCollectionUpdate')
    .addToUi();
...
ui.createMenu('      *Drafting')
    .addSeparator()  // FIXED: Added the missing dot here
    .addItem('✂️ Slice Base Topic', 'parseBaseTopics')
    .addItem('🚙 Format Visit Details', 'parseVisitDetails')
    .addItem('✨ Clean Outline', 'cleanUpOutline')
    .addItem('🔗 Add ### URL to Docs', 'batchFormatDraftDocuments')
    .addSeparator()
    .addSeparator()
    .addItem('🌈 Transfer Drafts', 'batchSendAllToEditors')
    .addSeparator()
    .addSeparator()
    .addSeparator()
    .addItem('➗ Separate Topic List to Cells', 'topicsToCells') 
    .addItem('🔪 Divide Topic Definer', 'divideTopicDefiner')
    .addSeparator()
    .addSeparator()
    .addItem('📥 Collect 50 Topics', 'collectTopics')
    .addSeparator()
    .addSeparator()
    .addItem('❌❌ Delete No Images', 'deleteNoImages')  // NEW ITEM
    .addSeparator()
    .addSeparator()
        .addItem('👤 Name Claude Drafts', 'assignTopics')
    .addSeparator()
    .addItem('📋 Send to Drafter Basic', 'transferToBasic')
    .addSeparator()
    .addToUi();
...
// -----------------------------------------------------------
// TEST FUNCTION - Run this to verify Shutterstock regex fix
// -----------------------------------------------------------
function testShutterstockRegex() {
  const testCases = [
    // Should PASS - extract the ID correctly
    { url: 'https://www.shutterstock.com/image-photo/columbus-ohio-august-3-2019-city-1510993499', expected: '1510993499' },
    { url: 'https://www.shutterstock.com/image-photo/columbus-ohio-august-3-2019-city-1510993499?trackingId=6f85e591-e6c9-4039-9893-0aca18bf83f2', expected: '1510993499' },
    { url: 'https://www.shutterstock.com/image-photo/sunset-ocean-123456789', expected: '123456789' },
    { url: 'https://www.shutterstock.com/image-photo/sunset-ocean-123456789?src=search', expected: '123456789' },
    { url: 'https://www.shutterstock.com/image-photo/sunset-ocean-123456789#reviews', expected: '123456789' },
    { url: 'https://www.shutterstock.com/image-photo/sunset-ocean-123456789/', expected: '123456789' },
    { url: 'https://www.shutterstock.com/image-vector/graphic-design-987654321', expected: '987654321' },
    { url: 'https://www.shutterstock.com/image-illustration/artwork-111222333?src=search&pos=1', expected: '111222333' },
    { url: 'https://www.shutterstock.com/image-photo/old-photo-5', expected: '5' },
    { url: 'https://www.shutterstock.com/image-photo/test-1?tracking=abc', expected: '1' },
  ];

  let passed = 0;
  let failed = 0;

  Logger.log('=== SHUTTERSTOCK REGEX TEST RESULTS ===\n');

  testCases.forEach(function(test, index) {
    const result = extractStockPhotoIDFromURL(test.url);
    const status = result === test.expected ? 'PASS' : 'FAIL';

    if (status === 'PASS') {
      passed++;
      Logger.log((index + 1) + '. PASS: Got "' + result + '"');
    } else {
      failed++;
      Logger.log((index + 1) + '. FAIL: Expected "' + test.expected + '", got "' + result + '"');
  ...
  if (failed === 0) {
    ...
    Logger.log('\n Some tests failed. Please review the regex.');
  }
...
// -----------------------------------------------------------
// TEST FUNCTION - Compare both Wikimedia extraction methods
// -----------------------------------------------------------
function testWikimediaExtraction() {
  const testUrls = [
    'https://commons.wikimedia.org/wiki/File:Trinity_Test_-_Oppenheimer_and_Groves_at_Ground_Zero_001.jpg',
    'https://commons.wikimedia.org/wiki/File:Trinity_Test_-_Installation_of_Jumbo_prior_to_the_test.jpg',
    'https://commons.wikimedia.org/wiki/File:Trinity_blast_10sec.jpg',
    'https://commons.wikimedia.org/wiki/File:LTG_Leslie_R._Groves_(cropped).jpg',
  ];

  Logger.log('=== WIKIMEDIA EXTRACTION TEST ===\n');
  Logger.log('Testing ' + testUrls.length + ' URLs with both methods...\n');

  const results = [];

  testUrls.forEach(function(url, index) {
    Logger.log('--- Test ' + (index + 1) + ' ---');
    Logger.log('URL: ' + url.split('/wiki/')[1]);

    // Method 1: API-based
    Logger.log('\n[API Method]');
    const apiResult = extractWikiMetadataAPI(url);
    Logger.log('  Author: ' + (apiResult.username || '(empty)'));
    Logger.log('  Description: ' + (apiResult.description ? apiResult.description.substring(0, 60) + '...' : '(empty)'));
    Logger.log('  License: ' + (apiResult.licenseType || '(empty)'));

    // Method 2: Improved Regex (need to fetch HTML first)
    Logger.log('\n[Improved Regex Method]');
    try {
      const response = UrlFetchApp.fetch(url, {
        muteHttpExceptions: true,
        headers: { "User-Agent": "WIYS-Workflow/1.0" }
      });

      if (response.getResponseCode() === 200) {
        const html = response.getContentText();
        const regexResult = extractWikiMetadataImproved(url, html);
        Logger.log('  Author: ' + (regexResult.username || '(empty)'));
        Logger.log('  Description: ' + (regexResult.description ? regexResult.description.substring(0, 60) + '...' : '(empty)'));
        Logger.log('  License: ' + (regexResult.licenseType || '(empty)'));
        ...
        Logger.log('  Failed to fetch HTML: ' + response.getResponseCode());
      }
    } catch (e) {
      Logger.log('  Error fetching HTML: ' + e.message);
    }
  ...
  results.forEach(function(r) {
    if (r.api.username || r.api.description) apiSuccess++;
    if (r.regex.username || r.regex.description) regexSuccess++;
  });

  Logger.log('API method extracted data from: ' + apiSuccess + '/' + results.length + ' URLs');
  Logger.log('Regex method extracted data from: ' + regexSuccess + '/' + results.length + ' URLs');
  Logger.log('\nRecommendation: Use ' + (apiSuccess >= regexSuccess ? 'API' : 'Regex') + ' method as primary');
...
/**
 * Deletes all properties EXCEPT the important ones
 * Keeps: WP_USERNAME, WP_APP_PASSWORD, CLAUDE_API_KEY
 */
function cleanupOldProperties() {
  var scriptProps = PropertiesService.getScriptProperties();
  var props = scriptProps.getProperties();
  
  var keepList = ['WP_USERNAME', 'WP_APP_PASSWORD', 'CLAUDE_API_KEY'];
  var deleted = 0;
  ...
  SpreadsheetApp.getUi().alert('Cleanup Done', 'Deleted ' + deleted + ' old properties.\n\nYou can now add new properties.', SpreadsheetApp.getUi().ButtonSet.OK);
}


/**
 * Cleans up ALL cached tag/category properties
 * Keeps only: WP_USERNAME, WP_APP_PASSWORD, CLAUDE_API_KEY
 */
function cleanupAllCachedProperties() {
  var scriptProps = PropertiesService.getScriptProperties();
  var props = scriptProps.getProperties();
  ...
  Logger.log('Deleted: ' + deleted + ', Kept: ' + kept);
  SpreadsheetApp.getUi().alert(
    'Cleanup Done', 
    'Deleted ' + deleted + ' cached properties.\nKept ' + kept + ' essential properties.\n\nYou can now add CLAUDE_API_KEY.',
    SpreadsheetApp.getUi().ButtonSet.OK
  );
}
 ...
 */
/**
 * ONE-TIME: Populate Article Collection - LIVE UPDATES
 */
function populateArticleCollection() {
  var ui = SpreadsheetApp.getUi();
  ...
  var response = ui.alert(
    'Populate Article Collection',
    'Fetch all published articles from July 16, 2025 to today?\n\nYou will see rows appear in the sheet as it runs.',
    ui.ButtonSet.YES_NO
  );
  if (response !== ui.Button.YES) return;
  ...
  // Create sheet
  var sheet = createArticleCollectionSheet();
  ...
  while (keepGoing) {
    var queryUrl = CONFIG.ENDPOINTS.WP_POSTS + '?' + [
      'per_page=100',
      'page=' + page,
      'status=publish',
      '_embed=true',
      'after=2025-07-16T00:00:00',
      'orderby=date',
      'order=desc'
    ].join('&');
    ...
    if (response.getResponseCode() === 200) {
      var posts = JSON.parse(response.getContentText());
      Logger.log('Page ' + page + ': ' + posts.length + ' articles');
      
      if (posts.length === 0) {
        keepGoing = false;
      } else {
        // Process and write THIS PAGE immediately
        var pageData = [];
        ...
        for (var i = 0; i < posts.length; i++) {
          ...
          var state = '';
          if (post._embedded && post._embedded['wp:term']) {
            for (var t = 0; t < post._embedded['wp:term'].length && !state; t++) {
              var terms = post._embedded['wp:term'][t];
              for (var c = 0; terms && c < terms.length; c++) {
                if (terms[c].taxonomy === 'category') { state = terms[c].name; break; }
              }
            }
          }
          ...
          var tags = [];
          if (post._embedded && post._embedded['wp:term']) {
            for (var t = 0; t < post._embedded['wp:term'].length; t++) {
              var terms = post._embedded['wp:term'][t];
              for (var c = 0; terms && c < terms.length; c++) {
                if (terms[c].taxonomy === 'post_tag') tags.push(terms[c].name.toLowerCase());
              }
            }
          }
          
          // Title
          var title = post.title.rendered.replace(/&#8217;/g, "'").replace(/&#8216;/g, "'")
            .replace(/&#8220;/g, '"').replace(/&#8221;/g, '"').replace(/&#038;/g, '&').replace(/&amp;/g, '&');
          
          // Get category
          var category = getCategoryFromTitleAndTags(title, tags);
          
          pageData.push([state, category, title, post.link, thumb, 'No']);
        }
        
        // WRITE TO SHEET NOW - so you can see it
        sheet.getRange(currentRow, 1, pageData.length, 6).setValues(pageData);
        SpreadsheetApp.flush(); // Force update to show in sheet
        
        currentRow += pageData.length;
        totalArticles += pageData.length;
        
        Logger.log('Written ' + totalArticles + ' articles so far');
        
        if (posts.length < 100) keepGoing = false;
        else { page++; Utilities.sleep(500); }
      }
    } else {
  ...
  SpreadsheetApp.getUi().alert('Done!', 'Added ' + totalArticles + ' articles.', SpreadsheetApp.getUi().ButtonSet.OK);
}
