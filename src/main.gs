...
// SHEET LOCKING SYSTEM — stubbed out Feb 17 2026
// These are no-ops. Lock system parked for redesign.
// All batch functions still call these, so stubs prevent crashes.
function lockUploaderSheet(operationType) { /* no-op */ }
function unlockUploaderSheet(operationType) { /* no-op */ }
function isUploaderSheetLocked() { return null; /* always unlocked */ }
...
///// BATCH IMAGE METADATA
///// BATCH IMAGE METADATA
// [DELETED] Batch Image Metadata chain (11 functions) — removed Feb 17 2026
// batchGetImageMetadata, processImageMetadataWorkspaceLoop, selectSingleWorkspaceForMetadata,
// getArticlesForMetadataIndividual, showSingleWorkspaceMetadataPlan, processSingleWorkspaceMetadata,
// processMetadataArticlesInWorkspace, showMetadataResultsAndAskNext, askForNextMetadataWorkspace,
// extractMetadataForRow, showMetadataResults
// Snapshot: claude/pre-cleanup-snapshot-Xuq9x
...
/**
 * ============================================================================
 * SEND TO ARTICLE STATUS TRACKER
 * ============================================================================
 * Sends "Ready for Transfer" rows from Enhanced Drafter to Article Status Tracker.
 *
...
function transferDraftsToArticleTracker() {
  ...
  // Confirm
  var confirm = ui.alert('Send to Article Status Tracker',
    rowsToTransfer.length + ' row(s) ready to send to Article Status Tracker.\n\nProceed?',
    ui.ButtonSet.YES_NO);
  if (confirm !== ui.Button.YES) return;
...
/**
 * Extracts base topic + full intro from raw input for the Topic & Summary column.
 * Returns "base topic -- full intro" or just "base topic" if no intro found.
 */
function extractTopicSummary(baseTopic, rawInput) {
 ...
 * Uses 1 API call (getValues on columns A-M) — everything else is in memory.
 *
 * How it works:
 *   1. Read columns A through M in one getValues() call
 *   2. Detect merge type from the data pattern:
 *      - 13-col merge (workspace header): A has value, B through M all empty
 *      - 11-col merge (article title): A has value, B through K all empty, L has status
 *      - Content row: B has data (URL/doc link)
 *   3. Walk through data in memory to find workspaces and delete targets
 *
...
function bulkScanForDeleteTargets(uploaderSheet, selectedWorkspaces) {
  ...
  // ONE API call: read columns A through M for the entire sheet
  var data = uploaderSheet.getRange(1, 1, lastRow, 13).getValues();

  // Detect merge type for each row from the data pattern
  // Merged cells show value only in column A; columns B-K are empty for article titles,
  // columns B-M are empty for workspace headers. Content rows have data in column B.
  var mergeWidths = {};
  for (var row = 1; row <= lastRow; row++) {
    var d = data[row - 1];
    if (!d[0] || d[0].toString().trim() === '') continue;

    // Check if columns B through K (indices 1-10) are all empty
    var bToKEmpty = true;
    for (var col = 1; col <= 10; col++) {
      if (d[col] && d[col].toString().trim() !== '') {
        bToKEmpty = false;
        break;
      }
    }
    if (!bToKEmpty) continue;

    // B-K are empty → this is a merge row
    // If L and M are also empty → 13-col merge (workspace header / END ROW)
    // Otherwise → 11-col merge (article title)
    var lEmpty = !d[11] || d[11].toString().trim() === '';
    var mEmpty = !d[12] || d[12].toString().trim() === '';
    mergeWidths[row] = (lEmpty && mEmpty) ? 13 : 11;
  }

  // Uppercase selected workspace names for matching
  var selectedUpper = [];
  for (var i = 0; i < selectedWorkspaces.length; i++) {
    selectedUpper.push(selectedWorkspaces[i].toUpperCase());
  }

  // Find workspace boundaries using 13-col merges
  var workspaces = [];
  var currentWs = null;

  for (var row = 1; row <= lastRow; row++) {
    if (mergeWidths[row] !== 13) continue;

    var valueStr = data[row - 1][0].toString().trim().toUpperCase();
    ...
    if (valueStr.includes('END ROW')) {
      ...
      // Check if it matches a selected workspace
  ...
  // Find delete targets: 11-col merges with "Successful WP Upload" in column L
  var allArticles = [];
  ...
  for (var wi = 0; wi < workspaces.length; wi++) {
    ...
    for (var row = ws.startRow; row < ws.endRow; row++) {
      if (mergeWidths[row] !== 11) continue;

      var title = data[row - 1][0];
      var status = data[row - 1][11]; // Column L

      if (title && status === 'Successful WP Upload') {
        // Count rows until next merge
        var rowsToDelete = 1;
        var checkRow = row + 1;
...
function backfillWETColumnK() {
  ...
  // Read columns C through K — title in C (index 0), WP URL in D (index 1), K (index 8)
...
// scheduled/published) and copies them to Available WP Drafts.
// Run once from Apps Script editor: migrateWETToAvailable()
// SAFE: Does NOT delete rows from WET. You clean up WET manually after verifying.
// ============================================================================

function migrateWETToAvailable() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var ui = SpreadsheetApp.getUi();
  ...
  var wetSheet = ss.getSheetByName(CONFIG.SHEETS.WP_EDITING_TRACKER);
  if (!wetSheet) {
    ui.alert('Error', 'WP Editing Tracker sheet not found.', ui.ButtonSet.OK);
    return;
  }

  var availSheet = ss.getSheetByName(CONFIG.SHEETS.AVAILABLE_WP_DRAFTS);
  if (!availSheet) {
    ui.alert('Error', 'Available WP Drafts sheet not found.\n\nRun setupAvailableWPDrafts() first.', ui.ButtonSet.OK);
    return;
  }

  var wetLastRow = wetSheet.getLastRow();
  if (wetLastRow < 2) {
    ui.alert('No Data', 'WP Editing Tracker has no data rows.', ui.ButtonSet.OK);
    return;
  }

  // Read all WET data (row 2 onwards)
  ...
  var wetData = wetSheet.getRange(2, 1, wetLastRow - 1, 12).getValues();

  // Filter: only rows that have a WP URL (column D) and are NOT already scheduled/published
  // Skip rows with statuses that indicate they're done
  var skipStatuses = ['Scheduled', 'Published', 'Title Updated', 'DONE', 'Recorded'];
  var rowsToMigrate = [];

  for (var i = 0; i < wetData.length; i++) {
    var wpUrl = wetData[i][3]; // Column D (WP URL)
    var status = wetData[i][7]; // Column H (Article Status)

    // Must have a WP URL
    if (!wpUrl || wpUrl.toString().trim() === '') continue;

    // Skip rows with "done" statuses (they've already been processed)
    var statusStr = status.toString().trim();
    var shouldSkip = false;
    for (var s = 0; s < skipStatuses.length; s++) {
      if (statusStr.toLowerCase() === skipStatuses[s].toLowerCase()) {
        shouldSkip = true;
        break;
      }
    }
    ...
    rowsToMigrate.push({
      wetRow: i + 2, // actual sheet row number
      drafter: wetData[i][0],       // WET A → Available A
      articleType: wetData[i][1],    // WET B → Available E
      rawTitle: wetData[i][2],       // WET C → Available G
      wpUrl: wetData[i][3],          // WET D → Available H
      finalTitle: wetData[i][8],     // WET I → Available J
      baseTopic: wetData[i][9],      // WET J → Available K
      articleSummary: wetData[i][10], // WET K → Available L
      googleDocUrl: wetData[i][11]   // WET L → Available M
    });
  }

  if (rowsToMigrate.length === 0) {
    ui.alert('No Drafts Found', 'No eligible WordPress drafts found in WET.\n\nAll rows either have no WP URL or have a completed status.', ui.ButtonSet.OK);
    return;
  }

  // Show preview and confirm
  var preview = 'Found ' + rowsToMigrate.length + ' drafts to migrate from WET → Available WP Drafts.\n\n';
  for (var p = 0; p < Math.min(rowsToMigrate.length, 10); p++) {
    ...
    preview += '• Row ' + rowsToMigrate[p].wetRow + ': ' + title + '\n';
  }
  if (rowsToMigrate.length > 10) {
    preview += '... and ' + (rowsToMigrate.length - 10) + ' more\n';
  }
  preview += '\nThis will NOT delete anything from WET. Continue?';
  ...
  var startRow = availLastRow + 1;
  if (startRow < 2) startRow = 2; // Don't overwrite headers

  // Build output array
  var now = new Date();
  var dateStr = Utilities.formatDate(now, 'America/Phoenix', 'yyyy-MM-dd');
  var outputRows = [];

  for (var m = 0; m < rowsToMigrate.length; m++) {
    var article = rowsToMigrate[m];
    var row = new Array(15); // 15 columns A–O
    for (var c = 0; c < 15; c++) row[c] = ''; // initialize empty
    ...
    row[cols.BASE_TOPIC - 1] = article.baseTopic;
    row[cols.ARTICLE_SUMMARY - 1] = article.articleSummary;
    row[cols.GOOGLE_DOC_URL - 1] = article.googleDocUrl;
    // Tags (N) = blank (WET doesn't have Tags)
    // References (O) = blank (WET doesn't have References)

    outputRows.push(row);
  }

  // Bulk write to Available
  availSheet.getRange(startRow, 1, outputRows.length, 15).setValues(outputRows);
  SpreadsheetApp.flush();

  ui.alert(
    'Migration Complete',
    'Migrated ' + outputRows.length + ' articles from WET to Available WP Drafts.\n\n' +
